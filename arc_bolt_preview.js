...
const PALETTES = {
  ...
  storm: { // the approved Arc Bolt colors, verbatim
...
function fbmTex(size = 256, oct = 4) {
  return makeCanvas(size, (g, s) => {
    ...
    const lat = [];
    for (let o = 0; o < oct; o++) {
      const n = grid << o, layer = new Float32Array(n * n);
    ...
    for (let y = 0; y < s; y++) for (let x = 0; x < s; x++) {
      let v = 0, amp = 0.5;
      for (let o = 0; o < oct; o++) { v += sample(lat[o], x / s * lat[o].n, y / s * lat[o].n) * amp; amp *= 0.5; }
      const q = Math.floor(clamp01(v) * 255);
      const i = (y * s + x) * 4;
      img.data[i] = img.data[i + 1] = img.data[i + 2] = q; img.data[i + 3] = 255;
    }
...
function runeRingTex() {
  return makeCanvas(512, (g, s) => {
    ...
    g.shadowBlur = 6;
    for (const [r, w] of [[224, 9], [204, 4], [140, 6], [126, 3]]) {
      g.lineWidth = w; g.beginPath(); g.arc(c, c, r, 0, Math.PI * 2); g.stroke();
    }
    for (let k = 0; k < 8; k++) {
      const a = k / 8 * Math.PI * 2;
      const gx = c + Math.cos(a) * 172, gy = c + Math.sin(a) * 172;
...
function scorchTex() {
  return makeCanvas(256, (g, s) => {
    ...
    for (let i = 0; i < 340; i++) {
      ...
      const d = Math.pow(rnd(), 1.6) * c * 0.92;
      const rad = rr(6, 26) * (1 - d / c * 0.7);
      const alpha = 0.16 * (1 - d / c);
      g.fillStyle = `rgba(4,6,10,${alpha.toFixed(3)})`;
      g.beginPath(); g.arc(c + Math.cos(a) * d, c + Math.sin(a) * d, rad, 0, Math.PI * 2); g.fill();
    }
  });
}
...
function crackGlowTex() {
  return makeCanvas(256, (g, s) => {
    ...
    for (let k = 0; k < 7; k++) {
      ...
      let x = c, y = c, a = a0, d = 0;
      g.lineWidth = 3.2;
      g.beginPath(); g.moveTo(x, y);
      while (d < c * 0.86) {
        const step = rr(9, 20);
        a += rr(-0.55, 0.55);
        x += Math.cos(a) * step; y += Math.sin(a) * step; d += step;
        g.lineTo(x, y);
        if (rnd() < 0.3) {
          g.moveTo(x, y);
          g.lineTo(x + Math.cos(a + rr(-1.4, 1.4)) * rr(8, 22), y + Math.sin(a + rr(-1.4, 1.4)) * rr(8, 22));
          g.moveTo(x, y);
        }
  ...
  return makeCanvas(256, (g, s) => {
    ...
    for (let k = 0; k < 9; k++) {
      ...
      while (d < c * 0.88) {
        ...
        if (rnd() < 0.45) { // sharp side fracture
          g.moveTo(x, y);
...
function raysDecalTex() {
  return makeCanvas(256, (g, s) => {
    ...
    for (let k = 0; k < 16; k++) {
      ...
      const inner = c * 0.22, outer = c * (k % 2 ? 0.55 : 0.85) * rr(0.85, 1);
      g.lineWidth = k % 2 ? 2 : 4;
      g.beginPath();
...
function pawTex() {
  return makeCanvas(64, (g, s) => {
    ...
    g.beginPath(); g.ellipse(32, 40, 11, 9, 0, 0, Math.PI * 2); g.fill(); // pad
    for (const [x, y, r] of [[16, 24, 5], [27, 17, 5.5], [39, 17, 5.5], [48, 24, 5]]) {
      g.beginPath(); g.arc(x, y, r, 0, Math.PI * 2); g.fill(); // toes
    }
  });
}
...
function chipTex() {
  return makeCanvas(32, (g) => {
    g.fillStyle = 'rgba(255,255,255,0.95)';
    g.beginPath(); g.moveTo(16, 4); g.lineTo(27, 14); g.lineTo(20, 28); g.lineTo(7, 22); g.closePath(); g.fill();
  });
}
...
function smokeTex() {
  return makeCanvas(128, (g, s) => {
    for (let i = 0; i < 26; i++) {
      const a = rnd() * Math.PI * 2, d = Math.pow(rnd(), 1.4) * s * 0.3;
...
function flipbookSheet(style) {
  ...
  const tex = makeCanvas(GRID * CELL, (g) => {
    for (let f = 0; f < GRID * GRID; f++) {
      ...
      g.save();
      g.beginPath(); g.rect(cx - CELL / 2, cy - CELL / 2, CELL, CELL); g.clip();
...
const drawFlipFrame = {
  electric(g, cx, cy, t) { // the original: flash → eroding ring → filaments
    const flashA = Math.pow(Math.max(0, 1 - t * 2.6), 1.7);
    if (flashA > 0.01) {
      ...
      g.fillStyle = grad; g.beginPath(); g.arc(cx, cy, r0, 0, Math.PI * 2); g.fill();
    }
    const ringR = 8 + 50 * easeOutQuart(t);
    g.strokeStyle = `rgba(215,220,232,${(Math.pow(1 - t, 1.4) * 0.9).toFixed(3)})`;
    g.lineWidth = 6.5 * (1 - t) + 1.2;
    g.shadowColor = 'rgba(210,214,226,0.8)'; g.shadowBlur = 8;
    g.beginPath(); g.arc(cx, cy, ringR, 0, Math.PI * 2); g.stroke();
    g.shadowBlur = 0;
    const nFil = Math.max(2, Math.floor(11 * (1 - t * 0.7)));
    g.strokeStyle = `rgba(240,243,250,${Math.pow(1 - t, 1.2).toFixed(3)})`;
    g.lineWidth = 2.1 * (1 - t) + 0.5;
    for (let k = 0; k < nFil; k++) {
      const a = rnd() * Math.PI * 2;
      let x = cx + Math.cos(a) * 6, y = cy + Math.sin(a) * 6, ang = a, d = 0;
      g.beginPath(); g.moveTo(x, y);
  ...
  flame(g, cx, cy, t) { // roiling fire: blobs rise and lick upward
    ...
    for (let k = 0; k < n; k++) {
      ...
      const grad = g.createRadialGradient(x, y, 0, x, y, rad);
      grad.addColorStop(0, `rgba(255,255,255,${(Math.pow(1 - t, 1.1) * heat).toFixed(3)})`);
      grad.addColorStop(0.45, `rgba(200,200,205,${(Math.pow(1 - t, 1.3) * heat * 0.7).toFixed(3)})`);
      grad.addColorStop(1, 'rgba(120,120,125,0)');
      g.fillStyle = grad; g.beginPath(); g.arc(x, y, rad, 0, Math.PI * 2); g.fill();
    }
    if (t < 0.3) { // ignition flash
      const grad = g.createRadialGradient(cx, cy, 0, cx, cy, 26);
      grad.addColorStop(0, `rgba(255,255,255,${(1 - t / 0.3).toFixed(3)})`);
      grad.addColorStop(1, 'rgba(255,255,255,0)');
      g.fillStyle = grad; g.beginPath(); g.arc(cx, cy, 26, 0, Math.PI * 2); g.fill();
    }
    if (t > 0.4) drawSmokeBlobs(g, cx, cy - t * 26, t, 6, 34);
  },
  shatter(g, cx, cy, t) { // angular shards flying outward + glints
    const e = easeOutQuart(t);
    for (let k = 0; k < 10; k++) {
      const a = k / 10 * Math.PI * 2 + 0.4;
      const d = 6 + 52 * e * rr(0.75, 1);
      const x = cx + Math.cos(a) * d, y = cy + Math.sin(a) * d;
      const len = rr(8, 18) * (1 - t * 0.5), wid = len * 0.36;
      const rot = a + e * rr(1, 3);
      const alpha = Math.pow(1 - t, 1.15);
      g.save(); g.translate(x, y); g.rotate(rot);
      g.fillStyle = `rgba(235,240,248,${alpha.toFixed(3)})`;
      g.beginPath();
      g.moveTo(0, -len / 2); g.lineTo(wid / 2, len / 4); g.lineTo(0, len / 2); g.lineTo(-wid / 2, len / 4);
      g.closePath(); g.fill();
      g.restore();
    }
    // crystalline glints (plus signs)
    g.strokeStyle = `rgba(255,255,255,${Math.pow(1 - t, 1.6).toFixed(3)})`;
    g.lineWidth = 1.6;
    for (let k = 0; k < 5; k++) {
      const a = rnd() * Math.PI * 2, d = rr(4, 40 * e + 8);
      const x = cx + Math.cos(a) * d, y = cy + Math.sin(a) * d, s = rr(3, 7);
  ...
  radiance(g, cx, cy, t) { // gentle glory: soft bloom + rotating rays
    ...
    for (let k = 0; k < 12; k++) {
      ...
      const inner = glowR * 0.4, outer = glowR * rr(1.15, 1.7);
  ...
  void(g, cx, cy, t) { // implosion: ring contracts, tendrils curl inward
    const ringR = 56 * (1 - easeOutCubic(t)) + 7;
    const alpha = Math.pow(Math.min(1, t * 2.5), 0.8) * Math.pow(1 - t, 0.55);
    g.strokeStyle = `rgba(225,220,240,${alpha.toFixed(3)})`;
    g.lineWidth = 5 * (1 - t) + 1.6;
    g.shadowColor = 'rgba(200,190,230,0.8)'; g.shadowBlur = 9;
    g.beginPath(); g.arc(cx, cy, ringR, 0, Math.PI * 2); g.stroke();
    g.shadowBlur = 0;
    g.lineWidth = 1.8;
    for (let k = 0; k < 7; k++) {
    ...
    if (t > 0.72) { // collapse pop
      const p = (t - 0.72) / 0.28;
      const grad = g.createRadialGradient(cx, cy, 0, cx, cy, 30 * p + 6);
      grad.addColorStop(0, `rgba(255,255,255,${(1 - p).toFixed(3)})`);
      grad.addColorStop(1, 'rgba(255,255,255,0)');
      g.fillStyle = grad; g.beginPath(); g.arc(cx, cy, 30 * p + 6, 0, Math.PI * 2); g.fill();
    }
  },
  verdant(g, cx, cy, t) { // petals/leaves spiraling out on soft bloom
    const e = easeOutCubic(t);
    const grad = g.createRadialGradient(cx, cy, 0, cx, cy, 20 + 18 * e);
    grad.addColorStop(0, `rgba(245,248,242,${Math.pow(1 - t, 1.3).toFixed(3)})`);
    grad.addColorStop(1, 'rgba(210,220,205,0)');
    g.fillStyle = grad; g.beginPath(); g.arc(cx, cy, 20 + 18 * e, 0, Math.PI * 2); g.fill();
    for (let k = 0; k < 10; k++) {
      const a = k / 10 * Math.PI * 2 + e * 2.4 + k * 0.4;
      const d = 6 + 44 * e * rr(0.7, 1);
      const x = cx + Math.cos(a) * d, y = cy + Math.sin(a) * d;
      const alpha = Math.pow(1 - t, 1.1);
      g.save(); g.translate(x, y); g.rotate(a + e * 3);
      g.fillStyle = `rgba(235,242,230,${alpha.toFixed(3)})`;
      g.beginPath(); g.ellipse(0, 0, rr(6, 10) * (1 - t * 0.4), rr(2.5, 4), 0, 0, Math.PI * 2); g.fill();
      g.restore();
    }
  },
};
function drawSmokeBlobs(g, cx, cy, t, n, spread) {
  ...
  for (let k = 0; k < n; k++) {
    const a = rnd() * Math.PI * 2, d = rr(8, spread);
    const x = cx + Math.cos(a) * d, y = cy + Math.sin(a) * d;
    const rad = rr(12, 26);
    const grad = g.createRadialGradient(x, y, 0, x, y, rad);
...
function groundTexture() {
  const t = makeCanvas(512, (g, s) => {
    g.fillStyle = '#1a2130'; g.fillRect(0, 0, s, s);
    for (let i = 0; i < 2400; i++) {
      const v = rnd();
      g.fillStyle = v < 0.5 ? 'rgba(8,10,15,0.35)' : 'rgba(46,58,76,0.18)';
      g.beginPath(); g.arc(rnd() * s, rnd() * s, rr(0.6, 3.4), 0, Math.PI * 2); g.fill();
    }
    g.strokeStyle = 'rgba(58,74,100,0.14)'; g.lineWidth = 1.1;
    for (let i = 0; i < 26; i++) {
      let x = rnd() * s, y = rnd() * s, a = rnd() * Math.PI * 2;
      g.beginPath(); g.moveTo(x, y);
      for (let k = 0; k < 14; k++) { a += rr(-0.7, 0.7); x += Math.cos(a) * 12; y += Math.sin(a) * 12; g.lineTo(x, y); }
      g.stroke();
    }
  });
...
function totemTexture() {
  const t = makeCanvas(256, (g, s) => {
    g.fillStyle = '#241a10'; g.fillRect(0, 0, s, s);
    for (let i = 0; i < 40; i++) {
      g.strokeStyle = `rgba(14,10,6,${rr(0.2, 0.5).toFixed(2)})`;
      g.lineWidth = rr(1, 3);
      g.beginPath(); g.moveTo(rnd() * s, 0);
      g.bezierCurveTo(rnd() * s, s * 0.3, rnd() * s, s * 0.7, rnd() * s, s);
      g.stroke();
    }
    g.strokeStyle = 'rgba(150,210,255,0.9)'; g.lineWidth = 4; g.lineCap = 'round';
    for (let k = 0; k < 5; k++) {
      const y0 = 26 + k * 46;
...
const glName = glDbg ? String(renderer.getContext().getParameter(glDbg.UNMASKED_RENDERER_WEBGL)) : '';
const params = new URLSearchParams(location.search);
const LOW_GPU = (/swiftshader|llvmpipe|software|basic render/i.test(glName) || params.has('low')) && !params.has('full');
...
const DistortShader = {
  uniforms: {
    tDiffuse: { value: null },
    uRipples: { value: [new THREE.Vector4(0, 0, 9, 0), new THREE.Vector4(0, 0, 9, 0), new THREE.Vector4(0, 0, 9, 0), new THREE.Vector4(0, 0, 9, 0)] },
...
composer.addPass(new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), BLOOM.strength, BLOOM.radius, BLOOM.threshold));
...
function frostEdge(v) { frostVal = Math.max(frostVal, Math.min(v, 0.55)); } // corner kiss, never a whiteout
let gradeAmt = 0, caAmt = 0, radialAmt = 0;
...
function caPulse(v) { caAmt = Math.max(caAmt, v); }
function radialBurst(worldPos, v = 1) { radialWorld.copy(worldPos); radialAmt = Math.max(radialAmt, v); }
...
const TEX = {
  dot: softDotTex(), glow: glowRadialTex(), ribbon: ribbonTex(), star: starTex(),
  noise: fbmTex(256, 4), rune: runeRingTex(), scorch: scorchTex(),
  cracks: crackGlowTex(), shards: shardCrackTex(), rays: raysDecalTex(),
  paw: pawTex(), chip: chipTex(), smoke: smokeTex(),
};
...
{
  const groundMat = new THREE.MeshStandardMaterial({ map: groundTexture(), roughness: 0.96, metalness: 0 });
  ...
  const moonHalo = new THREE.Sprite(new THREE.SpriteMaterial({ map: TEX.glow, color: 0x9fb2e0, transparent: true, opacity: 0.35, blending: THREE.AdditiveBlending, depthWrite: false, fog: false }));
  moonHalo.scale.setScalar(26);
  moonHalo.position.set(-28, 27, -49);
  scene.add(moonHalo);
  const moonDisc = new THREE.Sprite(new THREE.SpriteMaterial({ map: TEX.dot, color: 0xe8eeff, transparent: true, opacity: 0.95, blending: THREE.AdditiveBlending, depthWrite: false, fog: false }));
  ...
  for (let i = 0; i < 260; i++) {
    const a = rnd() * Math.PI * 2, e = rr(0.12, 1.35), r = 100;
  ...
  scene.add(new THREE.Points(starGeo, new THREE.PointsMaterial({
    size: 0.5, map: TEX.dot, transparent: true, opacity: 0.65, color: 0xbdd4ff,
    blending: THREE.AdditiveBlending, depthWrite: false, fog: false,
  })));

  const rockMat = new THREE.MeshStandardMaterial({ color: 0x1d2430, roughness: 0.95, flatShading: true });
  for (let i = 0; i < 12; i++) {
    const rock = new THREE.Mesh(new THREE.DodecahedronGeometry(rr(0.5, 2.1), 0), rockMat);
    const a = rnd() * Math.PI * 2, d = rr(17, 31);
  ...
  for (let i = 0; i < 6; i++) {
    const sp = new THREE.Sprite(new THREE.SpriteMaterial({
      map: TEX.smoke, color: 0x3d5578, transparent: true, opacity: 0.05,
...
const _acol = new THREE.Color(), _acol2 = new THREE.Color();
let auroraFlash = 0;
const AUR_PTS = [0, 1, 2].map(() => Array.from({ length: 17 }, () => new THREE.Vector3())); // zero per-frame allocation
function updateAurora(dt) { // ribbon writer — runs between begin/commit
  const target = (state === 'windup' && ctx) ? _acol.copy(AURORA_BASE).lerp(ctx.pal.main, 0.55) : _acol.copy(AURORA_BASE);
...
function blobShadow(x, z, r) {
  const m = new THREE.Mesh(new THREE.CircleGeometry(r, 24),
    new THREE.MeshBasicMaterial({ map: TEX.glow, color: 0x000000, transparent: true, opacity: 0.5, depthWrite: false }));
...
const M = '/models';
const CLASS_VISUALS = {
  warrior: { url: `${M}/chars/players/knight.glb`, show: ['Knight_Helmet', 'Knight_Cape'], attach: [{ url: `${M}/weapons/sword_1handed.glb`, bone: 'handslot.r', acc: '1H_Sword' }], attacks: ['1H_Melee_Attack_Chop', '1H_Melee_Attack_Slice_Diagonal'] },
  paladin: { url: `${M}/chars/players/paladin.glb`, attach: [{ url: `${M}/weapons/axe_1handed.glb`, bone: 'handslot.r', acc: '1H_Axe' }], attacks: ['1H_Melee_Attack_Chop', '1H_Melee_Attack_Slice_Diagonal'] },
  hunter: { url: `${M}/chars/players/ranger.glb`, attach: [{ url: `${M}/weapons/crossbow_1handed.glb`, bone: 'handslot.r', acc: '1H_Crossbow' }], attacks: ['2H_Ranged_Shoot'] },
  rogue: { url: `${M}/chars/players/rogue.glb`, show: ['Rogue_Cape'], attach: [{ url: `${M}/weapons/dagger.glb`, bone: 'handslot.r', acc: 'Knife' }, { url: `${M}/weapons/dagger.glb`, bone: 'handslot.l', acc: 'Knife' }], attacks: ['Dualwield_Melee_Attack_Chop'] },
  priest: { url: `${M}/chars/players/mage.glb`, show: [], attach: [{ url: `${M}/weapons/staff.glb`, bone: 'handslot.r', acc: '2H_Staff' }], tint: 0xf0e9d6, tintStrength: 0.5, attacks: ['2H_Melee_Attack_Chop'] },
  mage: { url: `${M}/chars/players/mage.glb`, show: ['Mage_Cape'], attach: [{ url: `${M}/weapons/staff.glb`, bone: 'handslot.r', acc: '2H_Staff' }], attacks: ['2H_Melee_Attack_Chop'] },
  warlock: { url: `${M}/chars/players/mage.glb`, show: [], attach: [{ url: `${M}/weapons/wand.glb`, bone: 'handslot.r', acc: '1H_Wand' }, { url: `${M}/weapons/spellbook_open.glb`, bone: 'handslot.l', gripRef: 'Spellbook_open' }], tint: 0x8d5fd3, tintStrength: 0.45, attacks: ['Spellcast_Shoot'] },
  shaman: { url: `${M}/chars/players/barbarian.glb`, show: ['Barbarian_BearHat'], attach: [{ url: `${M}/weapons/axe_1handed.glb`, bone: 'handslot.r', acc: '1H_Axe' }], tint: 0x6f8fc9, tintStrength: 0.4, attacks: ['1H_Melee_Attack_Chop', '1H_Melee_Attack_Slice_Diagonal'] },
  druid: { url: `${M}/chars/players/druid.glb`, attach: [{ url: `${M}/weapons/staff.glb`, bone: 'handslot.r', acc: '2H_Staff' }], attacks: ['2H_Melee_Attack_Chop'] },
};
// authored grip fallbacks from assets.ts KAYKIT_HAND_GRIPS
const GRIPS = {
  '1H_Axe': { r: { p: [0.231697, 0.382471, 0], q: [0, 1, 0, 0], s: 0.622211 }, l: { p: [-0.231697, 0.382471, 0], q: [0, 0, 0, 1], s: 0.622211 } },
  '1H_Sword': { r: { p: [0, 0.555174, 0], q: [0, 1, 0, 0], s: 0.8876 }, l: { p: [0, 0.555174, 0], q: [0, 0, 0, 1], s: 0.8876 } },
  '1H_Crossbow': { r: { p: [0.2286, 0.0213, -0.0012], q: [0, 0.7071068, 0, 0.7071067], s: 0.6109 } },
  '2H_Staff': { r: { p: [-0.0427, 0.1769, 0], q: [0, 1, 0, 0], s: 1.0773 } },
  Knife: { r: { p: [-0.0095, 0.378, 0], q: [0, 1, 0, 0], s: 0.6029 }, l: { p: [0.0095, 0.378, 0], q: [0, 0, 0, 1], s: 0.6029 } },
...
async function loadRig(cls) {
  ...
  for (const att of def.attach ?? []) {
    const bone = model.getObjectByName(att.bone) ?? model.getObjectByName(att.bone.replace(/[[\].:/]/g, ''));
    if (!bone) continue;
    ...
    if (payload.children.length === 1) {
      ...
      payload.remove(child); holder.add(child);
      payload = holder;
    }
    const side = att.bone.replace(/[[\].:/]/g, '').endsWith('l') ? 'l' : 'r';
    const refName = side === 'l' && att.acc === 'Knife' ? 'Knife_Offhand' : (att.gripRef ?? att.acc);
    const ref = refName ? (model.getObjectByName(refName) ?? model.getObjectByName(refName.replace(/[[\].:/]/g, ''))) : null;
    if (ref) {
  ...
  const hold = (d) => {
    ...
    a.timeScale = 0.07;
    setTimeoutSim(d, () => { a.timeScale = prev; });
  };
  ...
  g.add(fallbackHand);
  const handL = model.getObjectByName('handslot.l') ?? model.getObjectByName('handslotl') ?? fallbackHand;
  const handR = model.getObjectByName('handslot.r') ?? model.getObjectByName('handslotr') ?? fallbackHand;

  const rig = {
    cls, group: g, model, mixer, play, hold, handL, handR, attacks: def.attacks, mats: rigMats,
...
function buildDummy() {
  ...
  const mk = (color, rough = 0.9) => {
    const m = new THREE.MeshStandardMaterial({ color, roughness: rough, emissive: 0x2f6bff, emissiveIntensity: 0 });
  ...
  head.position.y = 2.42; g.add(head);
  for (const dz of [-0.8, 0.8]) {
    const fist = new THREE.Mesh(new THREE.SphereGeometry(0.13, 10, 8), mk(0x54442c));
    fist.position.set(0, 1.82, dz); g.add(fist);
  }
  scene.add(g);

  const shellMat = new THREE.ShaderMaterial({
    uniforms: { uColor: { value: new THREE.Color(0x5aa4ff) }, uIntensity: { value: 0 }, uTime: { value: 0 }, uNoise: { value: TEX.noise } },
    vertexShader: `
  ...
  return { group: g, mats, shell, shellMat, barPivot, head, headBaseY: 2.42, hitPos: new THREE.Vector3(TARGET_POS.x - 0.25, 1.75, TARGET_POS.z) };
}
...
function buildTotems() {
  ...
  for (const dz of [-2.3, 2.3]) {
    const mat = new THREE.MeshStandardMaterial({
      ...
      emissive: 0x4d9fff, emissiveMap: tex, emissiveIntensity: 0.18,
    });
    const t = new THREE.Mesh(new THREE.CylinderGeometry(0.24, 0.3, 2.5, 10), mat);
    t.position.set(CASTER_POS.x - 1.9, 1.25, dz);
    scene.add(t);
    const cap = new THREE.Mesh(new THREE.ConeGeometry(0.34, 0.5, 8),
...
// ---------------------------------------------------------------- ribbons
const MAXV = 16384, MAXI = MAXV * 3; // index budget matches worst-case segments
class RibbonMesh {
  constructor() {
    ...
    this.geo.setAttribute('aFlow', new THREE.BufferAttribute(this.flow, 1).setUsage(THREE.DynamicDrawUsage));
  ...
  add(pts, width, color, hdrMul, taper = 1, flow = 0) {
    ...
    for (let k = 0; k < n; k++) {
      const p = pts[k];
      const prev = pts[Math.max(0, k - 1)], next = pts[Math.min(n - 1, k + 1)];
      ...
      const vi = (base + k * 2) * 3;
      this.pos[vi] = p.x + this._t1.x * w; this.pos[vi + 1] = p.y + this._t1.y * w; this.pos[vi + 2] = p.z + this._t1.z * w;
      this.pos[vi + 3] = p.x - this._t1.x * w; this.pos[vi + 4] = p.y - this._t1.y * w; this.pos[vi + 5] = p.z - this._t1.z * w;
      const r = color.r * hdrMul, g2 = color.g * hdrMul, b = color.b * hdrMul;
      this.col[vi] = r; this.col[vi + 1] = g2; this.col[vi + 2] = b;
      this.col[vi + 3] = r; this.col[vi + 4] = g2; this.col[vi + 5] = b;
      const ui = (base + k * 2) * 2;
      this.uv[ui] = u * 3; this.uv[ui + 1] = 0;
      this.uv[ui + 2] = u * 3; this.uv[ui + 3] = 1;
  ...
  commit() {
    ...
    if (emptyNow) { // nothing drawn — the stale GPU tail is invisible, skip the upload
      if (!this._wasEmpty) { this._wasEmpty = true; this.geo.setDrawRange(0, 0); }
      return;
    }
...
function genBolt(a, b, { jag = 0.14, subdiv = 5, branchP = 0.3, depth = 2, branchScale = 0.42 } = {}, out = [], intensity = 1) {
  ...
  if (depth > 0) {
    for (let k = 2; k < pts.length - 2; k += 2) {
      ...
      const dir = pts[k + 1].clone().sub(pts[k - 1]).normalize();
      ...
      genBolt(pts[k], end, { jag: jag * 1.3, subdiv: Math.max(2, subdiv - 2), branchP: branchP * 0.6, depth: depth - 1, branchScale }, out, intensity * 0.45);
    }
  }
  return out;
}
...
function spawnBoltFx(cfg) {
  boltFx.push({
    age: 0, lastGen: -1, cache: [],
    life: cfg.life ?? 0.2, flicker: cfg.flicker ?? 0.045,
    width: cfg.width ?? 0.1, glowWidth: cfg.glowWidth ?? (cfg.width ?? 0.1) * 3.2,
    hdr: cfg.hdr ?? 2.5, gen: cfg.gen, curve: cfg.curve ?? ((t) => Math.pow(1 - t, 0.7)),
    colCore: cfg.colCore, colGlow: cfg.colGlow,
  });
}
function updateBoltFx(dt) {
  for (let i = boltFx.length - 1; i >= 0; i--) {
    const fx = boltFx[i];
    fx.age += dt;
    if (fx.age >= fx.life) { boltFx.splice(i, 1); continue; }
    ...
    for (const line of fx.cache) {
      ribbons.add(line.pts, fx.glowWidth * line.intensity, fx.colGlow, HDR(1.3) * k * line.intensity, 1);
      ribbons.add(line.pts, fx.width * line.intensity, fx.colCore, HDR(fx.hdr) * k * line.intensity, 1);
    }
  }
}
...
class Pool {
  constructor(count, size, tex, gravity, fadePow = 1.4, bounce = false) {
    this.count = count; this.n = 0; this.gravity = gravity; this.fadePow = fadePow; this.bounce = bounce;
    this.p = new Float32Array(count * 3); this.vel = new Float32Array(count * 3);
    this.c = new Float32Array(count * 3); this.base = new Float32Array(count * 3);
  ...
  update(dt) {
    for (let i = 0; i < this.n; i++) {
      ...
      if (this.life[i] <= 0) {
        const l = --this.n;
        for (let k = 0; k < 3; k++) {
          this.p[i * 3 + k] = this.p[l * 3 + k]; this.vel[i * 3 + k] = this.vel[l * 3 + k];
          this.base[i * 3 + k] = this.base[l * 3 + k];
        }
        this.life[i] = this.life[l]; this.max[i] = this.max[l];
        i--; continue;
      }
      ...
      const f = Math.pow(clamp01(this.life[i] / this.max[i]), this.fadePow);
      this.c[i * 3] = this.base[i * 3] * f; this.c[i * 3 + 1] = this.base[i * 3 + 1] * f; this.c[i * 3 + 2] = this.base[i * 3 + 2] * f;
    }
    const emptyNow = this.n === 0;
    if (emptyNow) { // draw range 0 hides the stale tail — no upload needed
      if (!this._wasEmpty) { this._wasEmpty = true; this.geo.setDrawRange(0, 0); }
      return;
    }
    this._wasEmpty = false;
    ...
    pa.addUpdateRange(0, this.n * 3); ca.addUpdateRange(0, this.n * 3);
    pa.needsUpdate = true;
    ca.needsUpdate = true;
...
function spawnDebris(kind, pos, pal, count) {
  if (kind === 'shards') {
    for (let i = 0; i < count; i++) {
      const a = rnd() * Math.PI * 2, sp = rr(3, 8);
      debrisPool.spawn(pos.x, pos.y, pos.z,
        Math.cos(a) * sp, rr(2, 6), Math.sin(a) * sp, rr(0.8, 1.4),
        pal.ion.r * HDR(1.9), pal.ion.g * HDR(1.9), pal.ion.b * HDR(1.9));
    }
  } else if (kind === 'smolder') {
    for (let i = 0; i < count; i++) {
      const a = rnd() * Math.PI * 2, sp = rr(2, 6);
      smolderPool.spawn(pos.x, pos.y, pos.z,
        Math.cos(a) * sp, rr(1.5, 5), Math.sin(a) * sp, rr(1.4, 2.2),
        pal.main.r * HDR(2), pal.main.g * HDR(2), pal.main.b * HDR(2));
    }
  } else if (kind === 'chips') {
    for (let i = 0; i < count; i++) {
      const a = rnd() * Math.PI * 2, sp = rr(2.5, 7);
      debrisPool.spawn(pos.x, pos.y, pos.z,
        Math.cos(a) * sp, rr(2, 5), Math.sin(a) * sp, rr(0.6, 1.1),
        0.55, 0.5, 0.42); // dim stone/wood chips
    }
...
function burst(pool, x, y, z, n, speed, life, col, hdrMul, upBias = 0.4) {
  for (let i = 0; i < n; i++) {
    const a = rnd() * Math.PI * 2, e = rr(-0.5, 1) * Math.PI / 2;
    const s = speed * rr(0.35, 1);
    pool.spawn(x, y, z,
      Math.cos(a) * Math.cos(e) * s, (Math.sin(e) + upBias) * s, Math.sin(a) * Math.cos(e) * s,
      life * rr(0.5, 1), col.r * hdrMul, col.g * hdrMul, col.b * hdrMul);
  }
}
...
function spawnStreaks(pos, col, n, speed) {
  for (let i = 0; i < n; i++) {
    const a = rnd() * Math.PI * 2, e = rr(-0.2, 1.1) * Math.PI / 2;
    const s = speed * rr(0.5, 1.3);
    streaksList.push({
      p: pos.clone(),
      v: new THREE.Vector3(Math.cos(a) * Math.cos(e) * s, (Math.sin(e) + 0.35) * s, Math.sin(a) * Math.cos(e) * s),
      life: rr(0.18, 0.34), max: 0.34, col,
    });
  }
}
const _sPair = [new THREE.Vector3(), new THREE.Vector3()]; // reused — add() copies immediately
function updateStreaks(dt) { // runs between ribbons.begin/commit
  for (let i = streaksList.length - 1; i >= 0; i--) {
    const s = streaksList[i];
    s.life -= dt;
    if (s.life <= 0) { streaksList.splice(i, 1); continue; }
    s.v.y -= 16 * dt;
...
function spawnContactShadow(pos, r) {
  const m = new THREE.Mesh(new THREE.CircleGeometry(1, 24),
    new THREE.MeshBasicMaterial({ map: TEX.glow, color: 0x000000, transparent: true, opacity: 0, depthWrite: false }));
...
function updateContactShadows(dt) {
  for (let i = contactShadows.length - 1; i >= 0; i--) {
    ...
    if (t >= 1) { scene.remove(c.m); c.m.material.dispose(); c.m.geometry.dispose(); contactShadows.splice(i, 1); }
  }
}
...
const ringMatProto = new THREE.ShaderMaterial({
  uniforms: {
    uProgress: { value: 0 }, uColor: { value: new THREE.Color() }, uIntensity: { value: 1 },
...
function updateRings(dt) {
  for (let i = shockRings.length - 1; i >= 0; i--) {
    ...
    if (t >= 1) { scene.remove(r.m); r.mat.dispose(); r.m.geometry.dispose(); shockRings.splice(i, 1); }
  }
}

// ---------------------------------------------------------------- flipbook
const flipMat = new THREE.ShaderMaterial({
  uniforms: { uMap: { value: null }, uFrame: { value: 0 }, uOpacity: { value: 1 }, uTint: { value: new THREE.Color(1, 1, 1) }, uHdr: { value: 1 } },
  vertexShader: `
    varying vec2 vUv;
    void main() { vUv = uv; gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }`,
  fragmentShader: `
...
function updateFlipbooks(dt) {
  for (let i = flipbooks.length - 1; i >= 0; i--) {
    ...
    if (t >= 1) { scene.remove(f.m); f.mat.dispose(); f.m.geometry.dispose(); flipbooks.splice(i, 1); }
  }
}
...
function decalMaterial(map, hdrMul) {
  return new THREE.ShaderMaterial({
    uniforms: {
      uMap: { value: map }, uNoise: { value: TEX.noise }, uColor: { value: new THREE.Color(0x5aa4ff) },
      uDissolve: { value: 1 }, uHdr: { value: hdrMul }, uSpin: { value: 0 },
    },
    vertexShader: `
      varying vec2 vUv;
      void main() { vUv = uv; gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }`,
...
const scorchDark = new THREE.Mesh(new THREE.CircleGeometry(1.9, 32),
  new THREE.MeshBasicMaterial({ map: TEX.scorch, transparent: true, opacity: 0, depthWrite: false, color: 0x000000 }));
...
const shaftMatProto = new THREE.ShaderMaterial({
  uniforms: {
    uColor: { value: new THREE.Color() }, uIntensity: { value: 1 },
    uTime: { value: 0 }, uNoise: { value: null }, uFade: { value: 1 },
  },
  vertexShader: `
...
function spawnShaft(pos, { radius = 1.1, height = 13, color, dur = 0.55, intensity = 1 } = {}) {
  ...
  mat.uniforms.uIntensity.value = intensity;
  const m = new THREE.Mesh(new THREE.CylinderGeometry(radius * 0.38, radius, height, 20, 1, true), mat);
  m.position.set(pos.x, height / 2, pos.z);
  m.renderOrder = 5;
  scene.add(m);
  const rec = { m, mat, age: 0, dur, end() { if (this.dur > this.age + 0.3) this.dur = this.age + 0.3; } };
  shafts.push(rec);
  return rec;
}
function updateShafts(dt) {
  for (let i = shafts.length - 1; i >= 0; i--) {
    ...
    s.mat.uniforms.uFade.value = Math.min(1, s.age / 0.15) * (s.dur === Infinity ? 1 : Math.pow(1 - t, 1.1));
    if (t >= 1) { scene.remove(s.m); s.mat.dispose(); s.m.geometry.dispose(); shafts.splice(i, 1); }
  }
}
function clearShafts() { for (const s of shafts) s.end(); }
...
class LightPulsePool {
  ...
  pulse(pos, color, peak, dur) {
    ...
    best.age = 0; best.dur = dur; best.peak = peak;
  }
  update(dt) {
    for (const s of this.lights) {
      s.age += dt;
      const t = clamp01(s.age / s.dur);
      s.l.intensity = t >= 1 ? 0 : s.peak * (t < 0.1 ? easeOutCubic(t / 0.1) : easeOutExpo(1 - (t - 0.1) / 0.9));
    }
  }
}
...
const PROJ_DEFAULT = { fire: 'rock', blood: 'rock', frost: 'shard', moon: 'shard', shadow: 'wisp', venom: 'wisp' };
let projMesh = null;
function buildProjMesh(style, pal) {
  const g = new THREE.Group();
  const add = (geo, color, hdr, opts = {}) => {
    const m = new THREE.Mesh(geo, new THREE.MeshBasicMaterial({
      color: color.clone().multiplyScalar(HDR(hdr)), transparent: true, opacity: opts.opacity ?? 0.9,
  ...
  if (style === 'shard') {
    ...
    const core = new THREE.Mesh(new THREE.IcosahedronGeometry(0.17, 1),
      new THREE.MeshBasicMaterial({ color: 0x1c0e06, toneMapped: false })); // dark molten core
    g.add(core);
    add(new THREE.IcosahedronGeometry(0.2, 1), pal.main, 1.5, { opacity: 0.4 }); // heat shell
    g.userData.spin = 3.2;
  } else if (style === 'wisp') {
    for (let i = 0; i < 3; i++) {
      const s = add(new THREE.SphereGeometry(rr(0.08, 0.13), 10, 8), pal.main, 1.6, { opacity: 0.65 });
      s.userData.ph = rnd() * 6.28;
    }
    g.userData.writhe = true;
  } else if (style === 'arrow') {
    const shaft = new THREE.Mesh(new THREE.CylinderGeometry(0.014, 0.014, 0.55, 6),
...
function clearProjMesh() {
  if (!projMesh) return;
  projMesh.traverse((o) => { if (o.isMesh) { o.material.dispose(); o.geometry.dispose(); } });
...
function updateProjMesh(dt) {
  ...
  if (projMesh.userData.writhe) {
    projMesh.children.forEach((c, i) => {
      ...
      c.position.set(Math.sin(ph + i * 2.1) * 0.14, Math.cos(ph * 1.3 + i) * 0.12, Math.sin(ph * 0.7 + i * 4) * 0.14);
    });
  }
}
...
scene.add(orbShell);
function sprite(tex) {
  const s = new THREE.Sprite(new THREE.SpriteMaterial({ map: tex, color: 0xffffff, transparent: true, blending: THREE.AdditiveBlending, depthWrite: false, toneMapped: false }));
  s.visible = false; scene.add(s); return s;
}
const orbCore = sprite(TEX.glow), orbHalo = sprite(TEX.glow);
const releaseFlash = sprite(TEX.glow), impactFlash = sprite(TEX.glow);
const headCore = sprite(TEX.glow), headHalo = sprite(TEX.glow);
...
function spawnSmoke(pos, n, colorHex) {
  for (let i = 0; i < n; i++) {
    const sp = new THREE.Sprite(new THREE.SpriteMaterial({
      map: TEX.smoke, color: colorHex, transparent: true, opacity: 0,
      depthWrite: false,
    }));
    ...
    scene.add(sp);
    smokes.push({ sp, age: 0, dur: rr(1.7, 2.4), rise: rr(0.5, 0.9), spin: rr(-0.5, 0.5), grow: rr(0.9, 1.6) });
  }
}
function updateSmokes(dt) {
  for (let i = smokes.length - 1; i >= 0; i--) {
    ...
    s.sp.material.opacity = 0.34 * easeInOutSine(t < 0.25 ? t / 0.25 : 1) * Math.pow(1 - t, 1.3);
    if (t >= 1) { scene.remove(s.sp); s.sp.material.dispose(); smokes.splice(i, 1); }
  }
}
...
function updateDrips(dt) {
  for (let i = drips.length - 1; i >= 0; i--) {
    const d = drips[i];
    d.until -= dt;
    if (d.until <= 0) { drips.splice(i, 1); continue; }
    if (rnd() < dt * d.rate) {
      const x = d.pos.x + rr(-0.4, 0.4), y = d.pos.y + rr(-0.8, 0.8), z = d.pos.z + rr(-0.4, 0.4);
      const c = d.pal.ion, h = HDR(1.6);
      if (d.style === 'rise') riserPool.spawn(x, y, z, rr(-0.15, 0.15), rr(0.4, 1), rr(-0.15, 0.15), rr(0.6, 1.2), c.r * h, c.g * h, c.b * h);
      else emberPool.spawn(x, y + 0.5, z, rr(-0.2, 0.2), rr(-0.4, -0.1), rr(-0.2, 0.2), rr(0.6, 1.1), c.r * h, c.g * h, c.b * h);
    }
  }
}
...
const SPIRIT_HEIGHT = {
  fox: 0.8, sheep: 1.1, wolf: 1.15, boar: 1.0, raptor: 1.7, stag: 2.0, bull: 2.2,
  bear: 2.5, hawk: 1.4, demon: 1.0, voidwalker: 2.4, ghost: 1.6, spider: 0.9,
};
async function spawnSpirit({ model, at, path = 'rise', scale = 1, dur = 1.5, dirX = 1, tint, dim, atKind = 'caster', follow = false }) {
  const url = SPIRIT_URLS[model];
  if (!url) return;
  const token = loopToken;
  let g;
  try { g = await loadGlb(url); } catch (e) { console.warn('[spirit] load failed', model, url, e?.message ?? e); return; }
  ...
  const overlaps = (path === 'rise' && atKind === 'caster') || path === 'swoop' || (path === 'pounce' && atKind === 'target');
  ...
  const mat = new THREE.MeshBasicMaterial({
    color: baseCol.multiplyScalar(HDR(ghostMul)), transparent: true, opacity: 0,
    blending: THREE.AdditiveBlending, depthWrite: false, toneMapped: false, // depth-tested: a body, not a sticker
  });
  ...
  const MOVE = findClip(['Gallop', 'Run', 'Fast_Flying', 'Walk', 'Move1 (jump)', 'Spider_Walk', 'Walking_A']);
  const IDLE = findClip(['Idle', 'Flying_Idle', 'Idle1', 'Idle_Combat', 'Spider_Idle']);
  const ATK = findClip(['Attack', 'Attack1 (marracca)', 'Attack_Headbutt', 'Punch', 'Spider_Attack']);
  const TEMPO = { bear: 0.85, bull: 1.0, wolf: 1.15, fox: 1.25, raptor: 1.3, sheep: 0.9, hawk: 1.1, stag: 0.95, demon: 0.9, voidwalker: 0.7, ghost: 0.6, spider: 1.1, boar: 1.1 }[model] ?? 1;
  ...
  if (atkA && moveA) {
    mixer.addEventListener('finished', (e) => { // bite done → back to stride
      if (e.action === atkA) { moveA.reset().play(); atkA.crossFadeTo(moveA, 0.12, false); }
    });
  }
  spirits.push({
    holder, mixer, mat, age: 0, dur, path, from: at.clone(), dirX, token, model, follow,
    uDissolve, pal, trail: [], prevPos: at.clone(), lastFoot: at.clone(), footSide: 1,
    chestY: targetH * 0.45, heightN: targetH,
    tempo: TEMPO, moveA, atkA, atkFired: false, yaw: 0,
  });
...
function updateSpirits(dt) {
  for (let i = spirits.length - 1; i >= 0; i--) {
    ...
    else if (sp.dur - sp.age < 0.35) sp.uDissolve.value = 0.1 + (1 - (sp.dur - sp.age) / 0.35) * 0.9;
    else sp.uDissolve.value = 0.1;
    const f = sp.from, d = sp.dirX;
    const fireAtk = () => {
    ...
    if (sp.follow && rig) { // dash afterimage: the spirit runs WITH the caster
      sp.holder.position.set(rig.group.position.x - 1.5 * d, f.y + Math.abs(Math.sin(sp.age * 9)) * 0.22, f.z + 0.6);
      if (rig.group.position.x >= MELEE_X - 0.6) fireAtk();
    } else if (sp.path === 'lunge') { // gather speed, strike as it passes through
      const u = easeInOutSine(t);
      sp.holder.position.set(f.x + (u * 6.4 - 2.8) * d, f.y, f.z);
      if (t > 0.38) fireAtk();
    } else if (sp.path === 'pounce') { // stalk → leap → land in the strike
      let x, y = f.y;
      if (t < 0.4) {
    ...
    } else if (sp.path === 'circle') { // considered three-quarter lap, eased in and out
      const u = easeInOutSine(t);
      const a = -Math.PI / 2 + u * Math.PI * 1.5;
      sp.holder.position.set(f.x + Math.cos(a) * 1.95, f.y, f.z + Math.sin(a) * 1.95);
    } else if (sp.path === 'swoop') { // committed dive, body pitched to the slope
      // low point clamped: talons rake just above head height, never through the body
      sp.holder.position.set(f.x + (t * 12 - 6) * d, Math.max(f.y + 5.5 - Math.sin(t * Math.PI) * 5, 1.55), f.z);
      ...
      if (sp.model === 'hawk' && sp.moveA) sp.moveA.timeScale = (t > 0.22 && t < 0.62 && !sp.atkFired) ? sp.tempo * 0.1 : sp.tempo;
    } else { // rise: calm, ceremonial presence — with a slow surveying turn
      let y = f.y + Math.sin(perfNow * 1.8) * 0.05;
      if (sp.model === 'sheep' && t > 0.4 && t < 0.55) y += Math.sin((t - 0.4) / 0.15 * Math.PI) * 0.4; // startled half-hop
    ...
    if (sp.path !== 'rise') {
      ...
      if (tmpA.lengthSq() > 1e-7) {
        ...
        if (sp.path === 'swoop') {
          ...
          sp.holder.rotation.z = Math.max(-0.55, Math.min(0.55, Math.atan2(tmpA.y, horiz + 1e-4) * 0.8));
        }
      }
    }
    // spectral shed
    if (rnd() < dt * 14) {
      ...
      emberPool.spawn(p.x + rr(-0.5, 0.5), p.y + rr(0.3, 1.4), p.z + rr(-0.5, 0.5),
        rr(-0.2, 0.2), rr(0.1, 0.5), rr(-0.2, 0.2), rr(0.4, 0.8),
        sp.mat.color.r * 0.7, sp.mat.color.g * 0.7, sp.mat.color.b * 0.7);
    }
    ...
    if (moving) {
      ...
      if (sp.trail.length > 3) ribbons.add(sp.trail, 0.55, sp.pal.ion, HDR(0.9) * clamp01(o), 0.85, 0.6);
      // spectral footprints on grounded paths
      if ((sp.path === 'lunge' || sp.path === 'circle') && sp.holder.position.distanceTo(sp.lastFoot) > 0.5) {
        tmpA.subVectors(sp.holder.position, sp.prevPos);
...
function spawnFootprint(pos, dir, side, pal, size = 0.3) {
  const m = new THREE.Mesh(new THREE.PlaneGeometry(size, size),
    new THREE.MeshBasicMaterial({ map: TEX.paw, color: pal.ion.clone().multiplyScalar(HDR(1.7)), transparent: true, opacity: 0.85, blending: THREE.AdditiveBlending, depthWrite: false, toneMapped: false }));
  m.rotation.x = -Math.PI / 2;
...
function updateFootprints(dt) {
  for (let i = footprints.length - 1; i >= 0; i--) {
    ...
    f.m.material.opacity = 0.85 * Math.pow(1 - t, 1.4);
    if (t >= 1) { scene.remove(f.m); f.m.material.dispose(); f.m.geometry.dispose(); footprints.splice(i, 1); }
  }
}
function clearFootprints() {
  for (const f of footprints) { scene.remove(f.m); f.m.material.dispose(); f.m.geometry.dispose(); }
  footprints.length = 0;
}
...
function startBuffOrbit(style, pal, dur, key = ctx?.id ?? 'anon', o = {}) {
  ...
  const spriteCount = o.n ?? (style === 'sparks' ? 3 : style === 'runes' ? 4 : style === 'halo' ? 3 : 0);
  const tex = TEX[ORBIT_TEX[o.tex] ?? (style === 'runes' ? 'star' : style === 'halo' ? 'star' : 'glow')];
  ...
  if (style === 'plates') { // orbiting shield facets (barriers, armors, barkskin)
    for (let i = 0; i < (o.n ?? 4); i++) {
      // soft-edged gradient panels — hard untextured quads read as debug geometry
      const m = new THREE.Mesh(new THREE.PlaneGeometry(0.62 * (o.size ?? 1), 0.82 * (o.size ?? 1)),
        new THREE.MeshBasicMaterial({ map: TEX.glow, color: pal.main.clone().multiplyScalar(HDR(1.9)), transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide, toneMapped: false }));
      scene.add(m);
      meshes.push(m);
    }
  }
  buffOrbits.set(key, { style, pal, until: dur, dur, sprites, meshes, phase: rnd() * 6.28, beat: 0, hist: [], o, tick: rr(0.5, 1.5) });
}
function disposeOrbit(bo) {
  for (const s of bo.sprites) { scene.remove(s); s.material.dispose(); }
  for (const m of bo.meshes) { scene.remove(m); m.material.dispose(); m.geometry.dispose(); }
}
function clearBuffOrbit(key) {
  if (key !== undefined) {
    const bo = buffOrbits.get(key);
    if (bo) { disposeOrbit(bo); buffOrbits.delete(key); }
    return;
  }
...
function updateOneOrbit(key, bo, dt) {
  bo.until -= dt;
  if (bo.until <= 0) { disposeOrbit(bo); buffOrbits.delete(key); return; }
  ...
  bo.tick -= dt;
  if (bo.tick <= 0 && sndOn) {
    bo.tick = (o.tickEvery ?? { leaves: 2.5, halo: 3, runes: 2.2, plates: 1.9, weaponGlow: 2.2, wings: 2.6 }[bo.style] ?? 999) * rr(0.85, 1.2);
    const pan = panOf(c.x);
    if (bo.style === 'leaves') sfx.tone2({ freq: rr(620, 720), dur: 0.4, gain: 0.045, pan, verb: 0.5 });
    else if (bo.style === 'halo') sfx.partials({ freqs: [1046, 1568], dur: 0.6, gain: 0.035, stagger: 0.03, pan, verb: 0.55 });
    else if (bo.style === 'runes') sfx.tone2({ freq: 220, dur: 0.5, gain: 0.04, type: 'sine', vibRate: 4, vibDepth: 0.02, pan, verb: 0.4 });
    else if (bo.style === 'plates') sfx.partials({ freqs: [820, 1240], dur: 0.18, gain: 0.03, stagger: 0.004, pan, verb: 0.3 });
    else if (bo.style === 'weaponGlow') sfx.noise2({ dur: 0.25, freq: 3400, gain: 0.03, type: 'bandpass', q: 6, pan, verb: 0.3 });
    else if (bo.style === 'wings') sfx.noise2({ dur: 0.5, freq: 900, sweep: 500, gain: 0.035, type: 'bandpass', q: 1.2, attack: 0.15, pan, verb: 0.4 });
  }
  if (bo.style === 'sparks' || bo.style === 'runes' || bo.style === 'halo') {
    ...
    bo.sprites.forEach((s, k) => {
      const a = bo.phase + t * (bo.style === 'sparks' ? 2.6 : bo.style === 'halo' ? 0.8 : 1.1) * rate + k / bo.sprites.length * Math.PI * 2;
    ...
    if (bo.style === 'sparks' && rnd() < dt * 8) {
      ...
      const a = bo.sprites[i].position.clone(), b = bo.sprites[j].position.clone();
      spawnBoltFx({
        life: 0.08, flicker: 0.04, width: 0.025, hdr: 2.6,
        colCore: pal.core, colGlow: pal.main,
        gen: () => genBolt(a, b, { jag: 0.3, subdiv: 3, branchP: 0.1, depth: 0 }),
      });
    }
  } else if (bo.style === 'leaves') {
    if (rnd() < dt * (o.density ?? 12)) {
      const a = rnd() * Math.PI * 2, r = rr(0.4, o.spread ?? 0.9);
      riserPool.spawn(c.x + Math.cos(a) * r, rr(0.1, 1.6), c.z + Math.sin(a) * r,
        Math.cos(a + 1.8) * 0.5, rr(0.4, 0.9) * (o.up ?? 1), Math.sin(a + 1.8) * 0.5, rr(0.9, 1.5),
        pal.ion.r * HDR(1.6) * fade, pal.ion.g * HDR(1.6) * fade, pal.ion.b * HDR(1.6) * fade);
    }
  } else if (bo.style === 'wings') { // spectral pinions with a translucent membrane
    const ribs = o.ribs ?? 6;
    const span = o.span ?? 1;
    const flap = Math.sin(t * (o.flapRate ?? 1.3)) * 0.3;
    for (const side of [-1, 1]) {
      let innerTip = null, outerTip = null;
      for (let f = 0; f < ribs; f++) {
        ...
        ribbons.add(smoothArc(rootP, tip, 0.16 * side, 8, 0.14), 0.18 * (1 - u * 0.45), pal.ion, HDR(2.4) * fade * (1 - u * 0.3), 0.7);
        if (f === 0) innerTip = tip;
        if (f === ribs - 1) outerTip = tip;
      }
      // membrane: soft wide spans so the fan reads as a surface, not quills
      ribbons.add(smoothArc(innerTip, outerTip, 0.2 * side, 8, 0), 0.5, pal.glow, HDR(0.7) * fade, 0.5);
      ribbons.add(smoothArc(new THREE.Vector3(c.x - 0.3, 1.75, c.z + side * 0.2), outerTip, 0.24 * side, 8, 0.1), 0.38, pal.glow, HDR(0.5) * fade, 0.5);
    }
    if (rnd() < dt * 6) {
      riserPool.spawn(c.x - rr(0.6, 1.2), rr(1.2, 2.4), c.z + rr(-1, 1), 0, -0.35, 0, rr(0.8, 1.4),
        pal.ion.r * fade, pal.ion.g * fade, pal.ion.b * fade); // drifting feather-motes
    }
  } else if (bo.style === 'weaponGlow') { // enchanted weapon shimmer + trail
    ...
    if (rnd() < dt * 10) {
      sparkPool.spawn(tmpA.x + rr(-0.1, 0.1), tmpA.y + rr(-0.15, 0.25), tmpA.z + rr(-0.1, 0.1),
        rr(-0.3, 0.3), rr(0.2, 0.7), rr(-0.3, 0.3), rr(0.25, 0.5),
        pal.core.r * HDR(2) * fade, pal.core.g * HDR(2) * fade, pal.core.b * HDR(2) * fade);
    }
    ...
    bo.meshes.forEach((m, k) => {
      ...
      m.position.set(c.x + Math.cos(a) * 0.9, 1.05 + Math.sin(t * 1.6 + k * 1.7) * 0.1, c.z + Math.sin(a) * 0.9);
  ...
  } else if (bo.style === 'heartbeat') { // pounding pulse rings off the chest
    bo.beat -= dt;
    if (bo.beat <= 0) {
      bo.beat = o.bpm ? 60 / o.bpm : 0.72; // frenzy buffs race, fortitude thumps slow
      spawnRing(new THREE.Vector3(c.x, 1.5, c.z), (o.ringR ?? 1.5), 0.5, pal.main, HDR(1.5) * fade, true);
      casterGlowV = Math.max(casterGlowV, 1.3 * fade);
      if (sndOn) sfx.heartbeat(panOf(c.x));
    }
  } else if (bo.style === 'speedlines') { // wind-blur streaks — LEG band, under everything else
    for (let k = 0; k < 3; k++) {
      if (rnd() > 0.6) continue;
      const y = rr(0.3, 1.25);
...
function updateBurn(dt) {
  if (!burnFx) return;
  burnFx.until -= dt;
  if (burnFx.until <= 0) { burnFx = null; return; }
  const fade = Math.min(1, burnFx.until / 0.8);
  targetGlow = Math.max(targetGlow, (1.0 + Math.sin(perfNow * 8) * 0.3) * fade);
  if (rnd() < dt * 5) { // occasional flare
    const p2 = randomOnDummy();
    emberPool.spawn(p2.x, p2.y, p2.z, rr(-0.2, 0.2), burnFx.drip === 'rise' ? rr(0.5, 1.2) : rr(-0.6, -0.2), rr(-0.2, 0.2), rr(0.4, 0.8),
      burnFx.pal.main.r * HDR(1.8), burnFx.pal.main.g * HDR(1.8), burnFx.pal.main.b * HDR(1.8));
  }
  if (burnFx.palName === 'storm') { // residual storm: arcs keep crawling the victim
    burnFx.arcT -= dt;
    if (burnFx.arcT <= 0) {
      burnFx.arcT = rr(0.35, 0.6);
      const a1 = randomOnDummy(), a2 = randomOnDummy();
      spawnBoltFx({
        life: rr(0.12, 0.2), flicker: 0.04, width: 0.04, hdr: 2.4,
        colCore: burnFx.pal.core, colGlow: burnFx.pal.main,
        gen: () => genBolt(a1, a2, { jag: 0.3, subdiv: 3, branchP: 0.2, depth: 0 }),
      });
      burst(sparkPool, dummy.hitPos.x, dummy.hitPos.y, dummy.hitPos.z, 3, 3, 0.3, burnFx.pal.ion, HDR(2), 0.3);
      if (!burnFx.grumbled && burnFx.until < burnFx.dur * 0.5) { // the storm grumbles once, high above
        burnFx.grumbled = true;
        lightPulses.pulse(new THREE.Vector3(dummy.hitPos.x, 8, dummy.hitPos.z), burnFx.pal.ion, 90, 0.5);
      }
    }
  }
}
...
function spawnOrbitStars(center, pal, dur, n = 5) {
  const sprites = [];
  for (let i = 0; i < n; i++) {
    const s = new THREE.Sprite(new THREE.SpriteMaterial({ map: TEX.star, color: pal.core.clone().multiplyScalar(HDR(1.8)), transparent: true, blending: THREE.AdditiveBlending, depthWrite: false, toneMapped: false }));
...
function updateOrbiters(dt) {
  for (let i = orbiters.length - 1; i >= 0; i--) {
    ...
    o.sprites.forEach((s, k) => {
      ...
      s.position.set(o.center.x + Math.cos(a) * 0.55, o.center.y + Math.sin(o.age * 3 + k) * 0.06, o.center.z + Math.sin(a) * 0.55);
      s.material.opacity = fade;
    });
    if (t >= 1) { for (const s of o.sprites) { scene.remove(s); s.material.dispose(); } orbiters.splice(i, 1); }
  }
}
...
function panOf(x) { return Math.max(-0.7, Math.min(0.7, x / 11)); }

// release-sample family per palette (12 palettes share 8 recorded releases)
const RELEASE_FAM = {
  fire: 'fire', blood: 'fire', frost: 'frost', moon: 'frost', arcane: 'arcane',
  shadow: 'shadow', venom: 'shadow', holy: 'holy', gold: 'holy', nature: 'nature',
  storm: 'storm', physical: 'physical',
};
const MOTIF_SAMPLE = {
  fissure: 'motif_fissure', gavel: 'motif_gavel', chains: 'motif_chains', vines: 'motif_vines',
  pillars: 'motif_pillars', crescents: 'motif_slashes', bladestorm: 'motif_slashes',
  implosion: 'motif_implosion', swarm: 'motif_swarm',
};

class Sfx {
  constructor() {
    this.ctx = null; this.master = null; this.verbIn = null; this.noiseBuf = null;
    this.bed = null; this.travelH = null; this.beamH = null;
    this.loops = new Set(); this.intensity = 1; this.volume = 1;
    ...
    this.samples = new Map(); this.packLoaded = false; this.sampleMode = false;
  }
  async loadPack() {
    if (this.packLoaded || !this.ctx) return;
    let raw = window.__SFX_PACK__;
    if (!raw) {
      try { const r = await fetch('/sfx_pack.json'); if (r.ok) raw = await r.json(); } catch { /* synth-only */ }
    }
    if (!raw) return;
    const b64buf = (b64) => {
      const s = atob(b64);
  ...
  _beefCurve(amount) { // cached soft-saturation curves — crunch without fizz
    ...
    if (!c) {
      ...
      for (let i = 0; i < 1024; i++) { const x = i / 511.5 - 1; c[i] = Math.tanh(k * x) / norm; }
      this._curves.set(key, c);
    }
    return c;
  }
  sample(id, { pan = 0, gain = 1, verb = 0.2, rate = 0, delay = 0, beef = 0.12, subOct = false, soft = false } = {}) {
    ...
    src.buffer = e.bufs[k];
    src.playbackRate.value = (rate || rr(0.95, 1.06)) * Math.max(0.55, Math.pow(timeScale, 0.6));
    const g = this.ctx.createGain();
    g.gain.value = gain * e.gains[k] * this.intensity;
    const out = this._out(pan, verb);
    let dest = out;
    if (beef > 0.02) { // the beef bus: saturation drive + low-shelf body + presence bite
      const sh = this.ctx.createWaveShaper();
      sh.curve = this._beefCurve(beef);
      const shelf = this.ctx.createBiquadFilter();
      shelf.type = 'lowshelf'; shelf.frequency.value = 170; shelf.gain.value = 3 + beef * 5;
      let tail = shelf;
      if (soft) { // screech control: shave the piercing top instead of boosting it
        const lp = this.ctx.createBiquadFilter();
        lp.type = 'lowpass'; lp.frequency.value = 4200; lp.Q.value = 0.6;
        shelf.connect(lp); tail = lp;
      } else {
        const pres = this.ctx.createBiquadFilter();
        pres.type = 'peaking'; pres.frequency.value = 2700; pres.Q.value = 0.9; pres.gain.value = 1.5 + beef * 3;
        shelf.connect(pres); tail = pres;
      }
      sh.connect(shelf); tail.connect(out);
    ...
    src.start(t);
    if (subOct) { // octave-down double of the same take — instant chest-cavity body
      ...
      s2.connect(lp); lp.connect(g2); g2.connect(dest);
      s2.start(t);
    }
    return true;
  }
  enable() {
    if (!this.ctx) {
      ...
      comp.attack.value = 0.011; comp.release.value = 0.3; // slower attack — transients PUNCH through
      ...
      const verbHp = ctx.createBiquadFilter(); verbHp.type = 'highpass'; verbHp.frequency.value = 200; verbHp.Q.value = 0.7;
      ...
      this.verbIn.connect(verbHp); verbHp.connect(verb); verb.connect(wet); wet.connect(this.master);
  ...
  get on() { return !!this.ctx && sndOn; }
  _applyMaster(tc = 0.02) { if (this.master) this.master.gain.setTargetAtTime(0.4 * this.volume * (this._duckF ?? 1), this._now(), tc); }
  setVolume(v) { this.volume = v; this._applyMaster(0.05); }
  setIntensity(m) { this.intensity = m; }
  ...
  _out(pan = 0, verb = 0.15) {
    ...
    if (verb > 0.01) { const s = this.ctx.createGain(); s.gain.value = verb; p.connect(s); s.connect(this.verbIn); }
    return g;
  }
  // ---- primitives ----------------------------------------------------------
  tone2({ freq, dur = 0.3, gain = 0.2, type = 'sine', slide = 0, delay = 0, pan = 0, verb = 0.15, attack = 0.01, vibRate = 0, vibDepth = 0, fmRatio = 0, fmDepth = 0 }) {
    if (!this.on) return;
    const t = this._now() + delay;
    ...
    if (vibRate) {
      const lfo = this.ctx.createOscillator(); lfo.frequency.value = vibRate;
      const lg = this.ctx.createGain(); lg.gain.value = vibDepth * freq;
      lfo.connect(lg); lg.connect(o.frequency); lfo.start(t); lfo.stop(t + dur + 0.1);
    }
    if (fmRatio) {
      const mod = this.ctx.createOscillator(); mod.frequency.value = freq * fmRatio;
      const mg = this.ctx.createGain(); mg.gain.value = fmDepth;
      mod.connect(mg); mg.connect(o.frequency); mod.start(t); mod.stop(t + dur + 0.1);
    }
    const g = this.ctx.createGain();
    g.gain.setValueAtTime(0.0001, t);
    g.gain.exponentialRampToValueAtTime(Math.max(0.001, gain * this.intensity), t + attack);
    g.gain.exponentialRampToValueAtTime(0.0001, t + dur);
    o.connect(g); g.connect(this._out(pan, verb));
    o.start(t); o.stop(t + dur + 0.1);
  }
  noise2({ dur = 0.3, freq = 800, gain = 0.3, type = 'lowpass', q = 1, sweep = 0, delay = 0, pan = 0, verb = 0.1, attack = 0.002 }) {
    if (!this.on) return;
    const t = this._now() + delay;
    const src = this.ctx.createBufferSource();
    ...
    g.gain.exponentialRampToValueAtTime(Math.max(0.001, gain * this.intensity), t + attack);
    g.gain.exponentialRampToValueAtTime(0.0001, t + dur);
    src.connect(f); f.connect(g); g.connect(this._out(pan, verb));
    src.start(t, Math.random()); src.stop(t + dur + 0.1);
  }
  partials({ freqs, dur = 0.8, gain = 0.2, type = 'sine', stagger = 0, delay = 0, pan = 0, verb = 0.35 }) {
    freqs.forEach((f, i) => this.tone2({ freq: f, dur: dur * (1 - i * 0.08), gain: gain / (1 + i * 0.7), type, delay: delay + i * stagger, pan, verb }));
  }
  ticks({ n = 6, over = 0.4, fLo = 3000, fHi = 6500, gain = 0.08, pan = 0, tone = false }) {
    for (let i = 0; i < n; i++) {
      const d = Math.random() * over;
      if (tone) this.tone2({ freq: fLo + Math.random() * (fHi - fLo), dur: 0.09, gain, delay: d, pan: pan + (Math.random() - 0.5) * 0.3, verb: 0.3 });
      else this.noise2({ dur: 0.04, freq: fLo + Math.random() * (fHi - fLo), gain, type: 'highpass', delay: d, pan: pan + (Math.random() - 0.5) * 0.3 });
    }
  }
  sub({ from = 110, to = 35, dur = 0.5, gain = 0.5, delay = 0, pan = 0 }) {
    this.tone2({ freq: from, slide: to, dur, gain: gain * (this._subScale ?? 1), type: 'sine', delay, pan, verb: 0, attack: 0.006 });
  }
  formant({ f0 = 120, dur = 0.4, gain = 0.35, bands = [[750, 5], [1250, 7]], slide = 0, pan = 0, verb = 0.2 }) {
    if (!this.on) return;
    const t = this._now();
    const o = this.ctx.createOscillator(); o.type = 'sawtooth'; o.frequency.setValueAtTime(f0, t);
  ...
  _loop(build) { // registered loop for slow-mo pitch tracking; returns handle
  ...
  frame(timeScale) { // slow-mo drops the pitch of every live loop
    if (!this.ctx || this.loops.size === 0) return;
    const mul = Math.max(0.5, Math.pow(timeScale, 0.6));
  ...
  duck(amount = 0.4, dur = 0.28) {
    ...
    setTimeoutSim(dur, () => { if (this._duckId === id) { this._duckF = 1; this._applyMaster(0.09); } }, true);
  }
  // ---- windup beds per charge style ---------------------------------------
  bedStart(style, palName, dur, pan = 0) {
    if (!this.on) return;
    this.bedStop();
    const t = this._now();
    this.bed = this._loop((h) => {
      const g = this.ctx.createGain(); h.gain = g;
      g.gain.setValueAtTime(0.0001, t);
      const out = this._out(pan, style === 'ascend' || style === 'runes' ? 0.45 : 0.15);
      g.connect(out);
      const mkOsc = (type, f) => { const o = this.ctx.createOscillator(); o.type = type; o.frequency.value = f; o.start(t); h.oscs.push({ o }); return o; };
      const mkSrc = (rate = 1) => { const s = this.ctx.createBufferSource(); s.buffer = this.noiseBuf; s.loop = true; s.playbackRate.value = rate; s.start(t, Math.random()); h.srcs.push({ s, base: rate }); return s; };
      if (style === 'orb' || style === 'weapon') {
        const o1 = mkOsc('sawtooth', 52), o2 = mkOsc('sawtooth', 52.8);
        ...
        f.frequency.setValueAtTime(280, t); f.frequency.exponentialRampToValueAtTime(style === 'weapon' ? 3400 : 2400, t + dur);
        o1.connect(f); o2.connect(f); f.connect(g);
        if (style === 'weapon') { // metallic shing rising with the enchantment
          const s = mkSrc(1);
          const bf = this.ctx.createBiquadFilter(); bf.type = 'bandpass'; bf.Q.value = 9;
          bf.frequency.setValueAtTime(2200, t); bf.frequency.exponentialRampToValueAtTime(4200, t + dur);
          const sg = this.ctx.createGain(); sg.gain.value = 0.35;
          s.connect(bf); bf.connect(sg); sg.connect(g);
        }
        g.gain.exponentialRampToValueAtTime(0.1 * this.intensity, t + dur * 0.85);
        h.bedTarget = 0.1;
      } else if (style === 'runes') {
        const o1 = mkOsc('sawtooth', 55), o2 = mkOsc('sawtooth', 82.5);
        const f = this.ctx.createBiquadFilter(); f.type = 'lowpass'; f.frequency.value = 320;
        o1.connect(f); o2.connect(f); f.connect(g);
        const s = mkSrc(0.9); // ritual whisper bed
        const wf = this.ctx.createBiquadFilter(); wf.type = 'highpass'; wf.frequency.value = 4200;
        const wg = this.ctx.createGain(); wg.gain.value = 0.06;
        const lfo = mkOsc('sine', 5.5); const lg = this.ctx.createGain(); lg.gain.value = 0.04;
        lfo.connect(lg); lg.connect(wg.gain);
        s.connect(wf); wf.connect(wg); wg.connect(g);
        ...
        setTimeoutSim(dur * 0.55, () => { if (this.bed === h) this.partials({ freqs: [523, 785], dur: 1.1, gain: 0.12, stagger: 0.02, pan, verb: 0.5 }); });
      } else if (style === 'vortex') {
        const s = mkSrc(1);
        const f = this.ctx.createBiquadFilter(); f.type = 'bandpass'; f.Q.value = 2.2;
        f.frequency.setValueAtTime(380, t); f.frequency.exponentialRampToValueAtTime(1600, t + dur);
        s.connect(f); f.connect(g);
        g.gain.exponentialRampToValueAtTime(0.22 * this.intensity, t + dur * 0.8);
        h.bedTarget = 0.22;
      } else if (style === 'ascend') {
        for (const [f, det] of [[220, 0], [277, 3], [330, -3], [440, 5]]) {
          const o = mkOsc('sine', f); o.detune.value = det;
        ...
        const s = mkSrc(0.7);
        const f = this.ctx.createBiquadFilter(); f.type = 'lowpass'; f.frequency.value = 130;
        s.connect(f); f.connect(g);
        const creak = mkOsc('sawtooth', 84);
        const cf = this.ctx.createBiquadFilter(); cf.type = 'lowpass'; cf.frequency.value = 240;
        const cg = this.ctx.createGain(); cg.gain.value = 0.1;
        const am = mkOsc('sine', 2.8); const ag = this.ctx.createGain(); ag.gain.value = 0.08;
        am.connect(ag); ag.connect(cg.gain);
        creak.connect(cf); cf.connect(cg); cg.connect(g);
  ...
  bedBreath(held) { // the held-breath beat silences the bed, then it slams back
  ...
  bedStop() { if (this.bed) { this.bed.stop(0.06); this.bed = null; } }
  // compat shims (old call names)
  chargeStart(dur) { this.bedStart(ctx ? ctx.windupStyle : 'orb', ctx ? ctx.palName : 'storm', dur, ctx && rig ? panOf(rig.group.position.x) : 0); }
  chargeStop() { this.bedStop(); }
  crackle() { this.noise2({ dur: 0.05, freq: 5200 + Math.random() * 2600, gain: 0.05, type: 'highpass', q: 0.7, pan: rig ? panOf(rig.group.position.x) : 0 }); }
  // ---- projectile travel ----------------------------------------------------
  travelStart(palName) {
    this.travelStop();
    if (!this.on) return;
    const t = this._now();
    if (palName === 'arcane' || palName === 'moon') { // magical projectiles hum, not hiss
      const f0 = palName === 'arcane' ? 880 : 520;
      this.travelH = this._loop((h) => {
        const g = this.ctx.createGain(); g.gain.value = 0.05; h.gain = g;
        for (const det of [0, 4.5]) {
          const o = this.ctx.createOscillator(); o.type = 'sine'; o.frequency.value = f0 + det;
          o.connect(g); o.start(t); h.oscs.push({ o });
        }
    ...
    const cfg = {
      fire: [700, 'lowpass', 0.09], blood: [600, 'lowpass', 0.08], frost: [6200, 'highpass', 0.05],
      shadow: [900, 'bandpass', 0.06], storm: [2600, 'bandpass', 0.06], venom: [700, 'bandpass', 0.07],
      holy: [5500, 'highpass', 0.04], gold: [5500, 'highpass', 0.04],
    }[palName] ?? [3100, 'bandpass', 0.05];
    this.travelH = this._loop((h) => {
      const s = this.ctx.createBufferSource(); s.buffer = this.noiseBuf; s.loop = true; s.playbackRate.value = 1;
      const f = this.ctx.createBiquadFilter(); f.type = cfg[1]; f.frequency.value = cfg[0]; f.Q.value = 2.2;
      const g = this.ctx.createGain(); g.gain.value = cfg[2]; h.gain = g;
      s.connect(f); f.connect(g); g.connect(this._out(0, 0.08));
      s.start(t, Math.random());
      h.srcs.push({ s, base: 1 });
    });
  }
  travelStop() { if (this.travelH) { this.travelH.stop(0.05); this.travelH = null; } }
  sizzleStart() { this.travelStart(ctx ? ctx.palName : 'storm'); }
  sizzleStop() { this.travelStop(); }
  // ---- beams: a hum that crescendos with the channel ------------------------
  beamStart(palName, pan = 0) {
    this.beamStop();
    if (!this.on) return;
    const t = this._now();
    this.beamH = this._loop((h) => {
      const o1 = this.ctx.createOscillator(); o1.type = 'sawtooth'; o1.frequency.value = 108;
      const o2 = this.ctx.createOscillator(); o2.type = 'sawtooth'; o2.frequency.value = 109.5;
      const f = this.ctx.createBiquadFilter(); f.type = 'lowpass'; f.frequency.value = 750; f.Q.value = 3;
      const g = this.ctx.createGain(); g.gain.setValueAtTime(0.0001, t);
      g.gain.exponentialRampToValueAtTime(0.09 * this.intensity, t + 0.25);
      o1.connect(f); o2.connect(f); f.connect(g); g.connect(this._out(pan, 0.2));
      o1.start(t); o2.start(t);
      h.oscs.push({ o: o1 }, { o: o2 });
      h.gain = g; h.filter = f;
    });
  }
  beamTick(i, total, pan = 0) {
    if (this.beamH?.filter) this.beamH.filter.frequency.setTargetAtTime(750 + i * 380, this._now(), 0.06);
    if (this.beamH?.gain) this.beamH.gain.gain.setTargetAtTime((0.09 + i * 0.03) * this.intensity, this._now(), 0.06);
    this.tone2({ freq: 320 * Math.pow(1.122, i), dur: 0.16, gain: 0.16, type: 'triangle', pan, verb: 0.25 });
  }
  ...
  impact(palName, { pan = 0, scale = 1, crit = false, finisher = false, lite = false, sample = null } = {}) {
    ...
    if (this.sampleMode && this.samples.has(sampleKey)) {
      ...
      if (finisher) this.partials({ freqs: [65, 98], dur: 1.2, gain: 0.3, type: 'triangle', pan, verb: 0.5 });
      return;
    }
    // when the crit adds its own deep sub, the recipe's sub steps aside
    this._subScale = lite ? 0 : (crit || finisher) ? 0.45 : 1;
    const P = {
      storm: () => { this.noise2({ dur: 0.1, freq: 7500, gain: 0.5 * I, type: 'highpass', delay: d, pan }); this.noise2({ dur: 0.9, freq: 420, gain: 0.42 * I, delay: d, pan, verb: 0.2 }); this.sub({ from: 130, to: I >= 1.2 ? 26 : 32, dur: 0.55 * (0.7 + 0.3 * I), gain: 0.55 * I, delay: d, pan }); this.ticks({ n: Math.round(6 * (0.6 + 0.5 * I)), over: 0.4, gain: 0.07 * I, pan }); },
      fire: () => { this.noise2({ dur: 0.5, freq: 520, sweep: 90, gain: 0.55 * I, delay: d, pan, verb: 0.15 }); this.sub({ from: 100, to: I >= 1.2 ? 32 : 40, dur: 0.45 * (0.7 + 0.3 * I), gain: 0.5 * I, delay: d, pan }); this.ticks({ n: Math.round(9 * (0.6 + 0.5 * I)), over: 0.55, fLo: 2800, fHi: 6000, gain: 0.09 * I, pan }); },
      blood: () => { this.noise2({ dur: 0.28, freq: 1200, sweep: 300, gain: 0.32 * I, type: 'bandpass', q: 3, delay: d, pan }); this.noise2({ dur: 0.22, freq: 180, sweep: 90, gain: 0.45 * I, delay: d, pan }); this.sub({ from: 85, to: 45, dur: 0.3, gain: 0.5 * I, delay: d, pan }); this.sub({ from: 70, to: 40, dur: 0.26, gain: 0.4 * I, delay: d + 0.26, pan }); }, // wet squelch signature
      frost: () => { this.ticks({ n: 11, over: 0.28, fLo: 1900, fHi: 5400, gain: 0.1 * I, pan, tone: true }); this.partials({ freqs: [1780, 2350, 3160], dur: 0.85, gain: 0.16 * I, stagger: 0.015, pan, verb: 0.45 }); this.noise2({ dur: 0.3, freq: 7000, gain: 0.28 * I, type: 'highpass', delay: d, pan }); },
      moon: () => { this.tone2({ freq: 1320, dur: 0.6, gain: 0.2 * I, delay: d, pan, verb: 0.45 }); this.noise2({ dur: 0.7, freq: 4200, gain: 0.1 * I, type: 'highpass', attack: 0.15, delay: d, pan, verb: 0.5 }); this.partials({ freqs: [1174, 880, 587], dur: 1.3, gain: 0.12 * I, stagger: 0.06, pan, verb: 0.55 }); }, // lunar glass: airy + falling cold partials, no FM zip (that's arcane's)
      arcane: () => { this.tone2({ freq: 700, slide: 1500, dur: 0.28, gain: 0.28 * I, fmRatio: 1.6, fmDepth: 380, delay: d, pan, verb: 0.25 }); this.partials({ freqs: [1320, 1980], dur: 0.6, gain: 0.13 * I, stagger: 0.03, pan, verb: 0.35 }); },
      shadow: () => { this.sub({ from: 160, to: 26, dur: 0.7, gain: 0.55 * I, delay: d, pan }); this.noise2({ dur: 0.7, freq: 380, gain: 0.35 * I, attack: 0.1, delay: d, pan, verb: 0.3 }); this.noise2({ dur: 0.5, freq: 5200, gain: 0.1 * I, type: 'highpass', delay: d + 0.08, pan, verb: 0.4 }); },
      venom: () => { for (let i = 0; i < 4; i++) this.tone2({ freq: 300 - i * 40, slide: 140, dur: 0.12, gain: 0.16 * I, delay: d + i * 0.06, pan }); this.noise2({ dur: 0.6, freq: 5200, gain: 0.14 * I, type: 'highpass', delay: d, pan }); },
      holy: () => { this.partials({ freqs: [523, 785, 1046, 1568], dur: 1.2, gain: 0.26 * I, stagger: 0.025, pan, verb: 0.55 }); this.noise2({ dur: 0.25, freq: 3000, gain: 0.12 * I, type: 'highpass', delay: d, pan, verb: 0.4 }); },
      gold: () => { this.partials({ freqs: [2140, 3420, 5580], dur: 0.42, gain: 0.2 * I, stagger: 0.02, pan, verb: 0.3 }); this.partials({ freqs: [523, 784], dur: 0.9, gain: 0.16 * I, stagger: 0.04, pan, verb: 0.45 }); this.sub({ from: 110, to: 60, dur: 0.35, gain: 0.3 * I, delay: d, pan }); }, // struck treasure, not a second choir
      nature: () => { this.tone2({ freq: 175, dur: 0.12, gain: 0.35 * I, type: 'triangle', delay: d, pan }); this.noise2({ dur: 0.45, freq: 1150, gain: 0.24 * I, type: 'bandpass', q: 2, delay: d, pan }); },
      physical: () => { this.tone2({ freq: 420, slide: 300, dur: 0.14, gain: 0.3 * I, type: 'triangle', delay: d, pan }); this.noise2({ dur: 0.16, freq: 260, gain: 0.5 * I, delay: d, pan }); this.sub({ from: 82, to: 50, dur: 0.24, gain: 0.42 * I, delay: d, pan }); this.partials({ freqs: [2870, 3350], dur: 0.15, gain: 0.13 * I, pan, verb: 0.2 }); }, // woody body knock leads
    };
    (P[palName] ?? P.physical)();
    // weight arrives with power, not loudness alone: big hits in sub-less
    // palettes earn a body thump
    if (I >= 1.15 && !lite && !['storm', 'fire', 'blood', 'shadow', 'physical', 'gold'].includes(palName)) {
  ...
  school(kind) { // legacy shim → nearest palette identity
    const map = { zap: 'storm', fire: 'fire', frost: 'frost', arcane: 'arcane', shadow: 'shadow', holy: 'holy', nature: 'nature', thwack: 'physical' };
    this.impact(map[kind] ?? 'physical', {});
  }
  impactExtra(crit) { if (crit) this.sub({ from: 60, to: 26, dur: 0.7, gain: 0.5 }); }
  release(palName, pan = 0) { // cast lets go: whoosh + spectral snap
    if (this.sample(`rel_${RELEASE_FAM[palName] ?? 'arcane'}`, { pan, gain: 0.85, verb: 0.2, beef: 0.18 })) return;
  ...
  whoosh(pan = 0, big = false) {
    if (this.sample(big ? 'whoosh_heavy' : 'whoosh_blade', { pan, gain: big ? 0.9 : 0.7, beef: big ? 0.25 : 0.15 })) return;
    this.noise2({ dur: big ? 0.22 : 0.13, freq: big ? 900 : 1300, sweep: big ? 300 : 600, gain: big ? 0.3 : 0.2, type: 'bandpass', q: 1.4, pan });
  }
  thunder(pan = 0) {
  ...
  ambienceStart() {
    ...
    this.amb = this._loop((h) => {
      const s = this.ctx.createBufferSource(); s.buffer = this.noiseBuf; s.loop = true; s.playbackRate.value = 0.5;
      const f = this.ctx.createBiquadFilter(); f.type = 'lowpass'; f.frequency.value = 240;
      const g = this.ctx.createGain(); g.gain.setValueAtTime(0.0001, t);
      g.gain.exponentialRampToValueAtTime(0.035, t + 1.2);
      h.gain = g;
      s.connect(f); f.connect(g); g.connect(this._out(0, 0.3));
      s.start(t, Math.random());
  ...
  heal(palName, pan = 0, sample = null) {
    ...
    this.noise2({ dur: 0.8, freq: 2600, gain: 0.06, type: 'highpass', attack: 0.25, pan, verb: 0.5 });
  }
  chime() { this.heal('holy', 0); }
  buffCast(style, palName, pan = 0) {
    if (this.sample(style === 'veil' ? 'buff_veil' : style === 'morph' ? 'buff_morph' : 'buff_raise', { pan, gain: 0.75, verb: 0.25 })) return;
    if (style === 'veil') { this.noise2({ dur: 0.5, freq: 3400, gain: 0.14, type: 'highpass', attack: 0.12, pan, verb: 0.3 }); return; }
    if (style === 'morph') {
      this.noise2({ dur: 0.32, freq: 500, sweep: 2400, gain: 0.22, type: 'bandpass', q: 2, pan }); // inward rush
      this.tone2({ freq: 880, slide: 1500, dur: 0.2, gain: 0.14, type: 'triangle', delay: 0.28, pan, verb: 0.3 });
      return;
    }
    // raise: ascending sparkle arpeggio in the palette's mood
    const seq = ['holy', 'gold'].includes(palName) ? [523, 659, 784] : ['shadow', 'venom'].includes(palName) ? [392, 466, 587] : [440, 554, 659];
    seq.forEach((f, i) => this.tone2({ freq: f, dur: 0.3, gain: 0.14, delay: i * 0.07, pan, verb: 0.35 }));
  }
  poof() {
    if (this.sample('poof', { gain: 0.8, verb: 0.25 })) return;
    this.noise2({ dur: 0.2, freq: 2600, gain: 0.3, type: 'highpass' }); this.tone2({ freq: 880, slide: 1500, dur: 0.18, gain: 0.14, type: 'triangle' });
  }
  ccPoof(pan = 0) { this.poof(); }
  baa(pan = 0) { // the sheep. non-negotiable.
    this.tone2({ freq: 620, slide: 540, dur: 0.2, gain: 0.24, type: 'sawtooth', fmRatio: 0.05, fmDepth: 210, pan, verb: 0.2 });
    this.tone2({ freq: 560, slide: 470, dur: 0.24, gain: 0.2, type: 'sawtooth', fmRatio: 0.05, fmDepth: 190, delay: 0.22, pan, verb: 0.2 });
  }
  shout(palName, pan = 0) { // an actual war-cry
    ...
    this.formant({ f0: 135, slide: 95, dur: 0.42, gain: 0.5, bands: [[760, 5], [1220, 7]], pan, verb: 0.3 });
    this.noise2({ dur: 0.3, freq: 600, gain: 0.28, pan });
    this.sub({ from: 95, to: 55, dur: 0.35, gain: 0.35, pan });
  }
  roar() { this.shout('blood', rig ? panOf(rig.group.position.x) : 0); }
  portal(pan = 0) {
    ...
    this.noise2({ dur: 0.9, freq: 1600, gain: 0.16, type: 'bandpass', q: 3, attack: 0.2, pan, verb: 0.45 });
  }
  ...
  heartbeat(pan = 0) { this.sub({ from: 62, to: 44, dur: 0.14, gain: 0.26, pan }); this.sub({ from: 56, to: 40, dur: 0.12, gain: 0.2, delay: 0.16, pan }); }
  distantRumble() { this.noise2({ dur: 1.1, freq: 210, gain: 0.05, verb: 0.5, attack: 0.3, pan: (Math.random() - 0.5) * 0.8 }); }
  spirit(model, pan = 0) { // creature calls — every apparition speaks
    // the screechers sit back in the mix; the growlers can stay forward
    const SPIRIT_VOICE = { raptor: [0.5, true], hawk: [0.45, true], fox: [0.45, true], ghost: [0.55, true], sheep: [0.7, false] };
    const [vol, softV] = SPIRIT_VOICE[model] ?? [0.8, false];
    if (this.sample(`spirit_${model}`, { pan, gain: vol, verb: 0.4, soft: softV })) return;
    switch (model) {
      case 'wolf':
        this.tone2({ freq: 400, slide: 640, dur: 0.5, gain: 0.2, type: 'sawtooth', vibRate: 5.5, vibDepth: 0.015, pan, verb: 0.5 });
        this.tone2({ freq: 620, slide: 370, dur: 0.55, gain: 0.17, type: 'sawtooth', vibRate: 5, vibDepth: 0.02, delay: 0.48, pan, verb: 0.55 });
        break;
      case 'bear':
        this.formant({ f0: 62, slide: 48, dur: 0.65, gain: 0.5, bands: [[290, 4], [620, 6]], pan, verb: 0.35 });
        this.sub({ from: 70, to: 40, dur: 0.5, gain: 0.35, pan });
        break;
      case 'raptor':
        this.tone2({ freq: 1500, slide: 820, dur: 0.18, gain: 0.2, type: 'square', pan, verb: 0.3 });
        this.noise2({ dur: 0.12, freq: 5200, gain: 0.12, type: 'highpass', delay: 0.05, pan });
        break;
      case 'hawk':
        this.tone2({ freq: 2150, slide: 1150, dur: 0.4, gain: 0.16, vibRate: 11, vibDepth: 0.03, pan, verb: 0.45 });
        break;
      case 'bull':
        this.noise2({ dur: 0.14, freq: 220, gain: 0.35, pan });
        this.sub({ from: 75, to: 45, dur: 0.3, gain: 0.4, delay: 0.1, pan });
        break;
      case 'sheep': this.baa(pan); break;
      case 'demon':
        this.formant({ f0: 55, dur: 0.7, gain: 0.45, bands: [[240, 4], [520, 6]], slide: 42, pan, verb: 0.4 });
        this.tone2({ freq: 63, dur: 0.6, gain: 0.2, type: 'sawtooth', pan, verb: 0.4 });
        break;
      case 'voidwalker':
        this.tone2({ freq: 112, slide: 66, dur: 1.1, gain: 0.24, vibRate: 3.2, vibDepth: 0.03, pan, verb: 0.6 });
        break;
      case 'ghost':
        this.tone2({ freq: 820, slide: 340, dur: 0.95, gain: 0.14, vibRate: 6, vibDepth: 0.05, pan, verb: 0.65 });
        break;
      default:
        this.partials({ freqs: [660, 990], dur: 0.7, gain: 0.09, stagger: 0.05, pan, verb: 0.5 });
    }
  }
  motif(name, pan = 0) { // set-piece foley
    if (MOTIF_SAMPLE[name] && this.sample(MOTIF_SAMPLE[name], { pan, gain: 0.85, verb: 0.3, beef: 0.3, subOct: name === 'gavel' || name === 'fissure' || name === 'pillars' })) return;
    switch (name) {
      case 'fissure': this.sub({ from: 90, to: 28, dur: 0.7, gain: 0.5, pan }); this.noise2({ dur: 0.65, freq: 160, gain: 0.38, pan }); this.ticks({ n: 5, over: 0.35, fLo: 900, fHi: 2200, gain: 0.1, pan }); break;
      case 'vines': for (let i = 0; i < 2; i++) this.tone2({ freq: 130 - i * 20, slide: 90, dur: 0.5, gain: 0.16, type: 'sawtooth', vibRate: 3, vibDepth: 0.06, delay: i * 0.18, pan, verb: 0.2 }); break;
      case 'chains': for (let i = 0; i < 3; i++) this.partials({ freqs: [820, 1240, 1660], dur: 0.22, gain: 0.14, stagger: 0.004, pan: pan + (i - 1) * 0.12, verb: 0.3 }); break;
      case 'swarm': this.noise2({ dur: 0.55, freq: 950, gain: 0.2, type: 'bandpass', q: 3, pan, verb: 0.25 }); break;
      case 'pillars': for (let i = 0; i < 5; i++) { this.noise2({ dur: 0.12, freq: 210, gain: 0.26, delay: i * 0.09, pan: pan + (i - 2) * 0.1 }); this.sub({ from: 72, to: 48, dur: 0.16, gain: 0.24, delay: i * 0.09, pan }); } break;
      case 'orbitals': break; // the sequenced impacts already ring
      case 'cross': this.partials({ freqs: [1046, 1568], dur: 0.9, gain: 0.16, stagger: 0.02, pan, verb: 0.5 }); break;
      case 'fountain': for (let i = 0; i < 6; i++) this.tone2({ freq: 380 + Math.random() * 420, slide: 700, dur: 0.09, gain: 0.09, delay: Math.random() * 0.4, pan, verb: 0.3 }); this.noise2({ dur: 0.7, freq: 1100, gain: 0.14, type: 'bandpass', q: 1.5, pan }); break;
      case 'crescents': for (let i = 0; i < 2; i++) this.noise2({ dur: 0.16, freq: 1900, sweep: 420, gain: 0.22, type: 'bandpass', q: 1.6, delay: i * 0.12, pan: pan + (i ? 0.2 : -0.2) }); break;
      case 'bladestorm': for (let i = 0; i < 3; i++) this.noise2({ dur: 0.15, freq: 1700, sweep: 500, gain: 0.18, type: 'bandpass', q: 1.5, delay: i * 0.14, pan: [-0.4, 0, 0.4][i] }); break;
      case 'implosion': this.noise2({ dur: 0.34, freq: 400, sweep: 2600, gain: 0.3, type: 'bandpass', q: 2, attack: 0.28, pan }); break;
      case 'barrier': for (let i = 0; i < 3; i++) this.partials({ freqs: [2140, 2680], dur: 0.16, gain: 0.1, delay: i * 0.06, pan, verb: 0.35 }); break;
      case 'gavel': this.sub({ from: 95, to: 30, dur: 0.5, gain: 0.5, delay: 0.2, pan }); this.partials({ freqs: [720, 1080, 1440], dur: 0.7, gain: 0.2, stagger: 0.005, delay: 0.2, pan, verb: 0.45 }); this.noise2({ dur: 0.2, freq: 300, gain: 0.35, delay: 0.2, pan }); break; // verdict lands
      case 'claws': for (let i = 0; i < 3; i++) this.noise2({ dur: 0.13, freq: 2100, sweep: 600, gain: 0.2, type: 'bandpass', q: 1.8, delay: i * 0.05, pan: pan + (i - 1) * 0.1 }); break;
    }
  }
  // very old positional-arg shims (a few direct call sites)
  noise(dur, freq, gain, type = 'lowpass', q = 1) { this.noise2({ dur, freq, gain, type, q }); }
  tone(freq, dur, gain, type = 'sine', slideTo = 0, delay = 0) { this.tone2({ freq, dur, gain, type, slide: slideTo, delay }); }
}
const sfx = new Sfx();
let sndOn = false;

// ---------------------------------------------------------------- camera feel
let trauma = 0, fovKick = 0;
const kickPos = new THREE.Vector3(), kickVel = new THREE.Vector3();
function addKick(dir, mag) { kickVel.addScaledVector(dir, mag); }
// caster lean + receiver recoil springs — bodies acknowledge the magic
function updateBodyFeel(dt, realDt) {
  let target = 0;
...
function updateCameraFeel(dt, realDt) {
...
let flashVal = 0;
function screenFlash(v) { flashVal = Math.max(flashVal, v); }
...
const simTimeouts = [];
let abilityToken = 0;
function setTimeoutSim(delay, fn, sticky = false) { simTimeouts.push({ left: delay, fn, tok: sticky ? -1 : abilityToken }); }
function updateSimTimeouts(dt) {
  for (let i = simTimeouts.length - 1; i >= 0; i--) {
    const e = simTimeouts[i];
    if (e.tok !== -1 && e.tok !== abilityToken) { simTimeouts.splice(i, 1); continue; }
    e.left -= dt;
    if (e.left <= 0) { simTimeouts.splice(i, 1); e.fn(); }
  }
}
...
const projPos = new THREE.Vector3(), projDir = new THREE.Vector3();
const trailHist = [];

let rig = null;           // active class rig
let ctx = null;           // current ability context
let state = 'idle', stateT = 0, loopCount = 0, isCrit = false;
let hitstopLeft = 0, timeScale = 1, slowMo = false, finisherOn = true, tourOn = false;
let userTimeScale = 1, cineOn = false, cineAngle = 0.6, cineAutoUntil = 0, cineShotIn = false, cineRadius = 9;
const cineFocus = new THREE.Vector3(0, 1.8, 0);
let comboOn = false, comboIx = 0;
let expoDipUntil = 0, impactFrames = 0;
let bodyLean = 0, bodyLeanVel = 0, dummyRot = 0, dummyRotVel = 0;
let barRot = 0, barVel = 0, headOff = 0, headVel = 0, kickSign = 1;
let skyFlickerNext = 1.2, coilPhase = 0, forkTimer = 0, groundArcTimer = 0;
let targetGlow = 0, shellGlow = 0, casterGlowV = 0, totemGlow = 0.18;
let scorchAlpha = 0, scorchMax = 0.85, crackDissolve = 1, runeDissolve = 1, portalDissolve = 1, portalHdr = 0;
let finisherFired = false, finisherTimer = 0, idleResumed = true, critSlowUntil = 0, critSlowStart = 0;
let comboWear = 0, lastResiduePal = null; // combo mode: accumulated damage read + residue interactions
...
function fallbackSpec(meta) {
  ...
  const id = meta.id;
  if (meta.channel) return { archetype: 'beam', beam: { dur: 2.0, ticks: 4, drain: effs.has('drainTick') } };
  if (effs.has('summonDemon') || effs.has('tamePet') || effs.has('dismissPet') || id === 'revive_pet') return { archetype: 'summon' };
  ...
  if (meta.targetType === 'self') return { archetype: 'buff', buff: { style: id.includes('stealth') || id.includes('prowl') ? 'veil' : (id.includes('form') || id.includes('wolf') ? 'morph' : 'raise') } };
  if (effs.has('aoeAttackPower') || effs.has('aoeAttackSpeed') || id.includes('shout') || id.includes('roar')) return { archetype: 'shout' };
  if ((effs.has('aoeDamage') || effs.has('aoeRoot')) && meta.range === 0) return { archetype: 'nova' };
  ...
  if (effs.has('weaponDamage') || effs.has('weaponStrike') || effs.has('finisherDamage') || (meta.range <= 6 && meta.school === 'physical')) {
...
const WINDUP_DEFAULT = {
  bolt: 'orb', burst: 'orb', strike: 'stance', nova: 'vortex', beam: 'orb',
  dot: 'orb', heal: 'ascend', buff: 'none', shout: 'stance', summon: 'runes',
  cc: 'orb', dash: 'stance',
};
...
function resolveSpec(cls, id) {
  ...
  const palName = (spec.palette ?? meta.school) in PALETTES ? (spec.palette ?? meta.school) : 'physical';
  ...
  return { cls, id, meta, spec, pal, palName, power: spec.power ?? 1, windup, windupStyle, arch: ARCHETYPES[spec.archetype] };
}
...
function focusPos(out) { // where the "impact" lands
...
function windupFx(p, dt) {
  ...
  const pal = ctx.pal;
  if (st === 'none') return;
  if (st === 'runes') { runeMesh.visible = true; placeRune(); return windupCharge(p, dt, true); }
  if (st === 'orb') return windupCharge(p, dt, false);
  if (st === 'weapon') { // enchantment crawls up the weapon
    ...
    if (rnd() < dt * (8 + p * 26)) { // sparks climbing the blade
      sparkPool.spawn(orbPos.x + rr(-0.12, 0.12), orbPos.y + rr(-0.3, 0.15), orbPos.z + rr(-0.12, 0.12),
        rr(-0.2, 0.2), rr(0.5, 1.2), rr(-0.2, 0.2), rr(0.25, 0.45),
        pal.ion.r * HDR(2), pal.ion.g * HDR(2), pal.ion.b * HDR(2));
    }
    if (rnd() < dt * p * 10) {
      const a = orbPos.clone(), b = orbPos.clone().add(new THREE.Vector3(rr(-0.5, 0.5), rr(-0.4, 0.5), rr(-0.5, 0.5)));
      spawnBoltFx({ life: 0.07, flicker: 0.035, width: 0.02, hdr: 2.6, colCore: pal.core, colGlow: pal.main, gen: () => genBolt(a, b, { jag: 0.3, subdiv: 3, branchP: 0.1, depth: 0 }) });
    }
    return;
  }
  if (st === 'vortex') { // power spirals inward around the body
    casterChest(orbPos);
    const c = rig.group.position;
    const rate = 40 * (0.3 + p);
    let n = rate * dt;
    while (n > 0) { if (n < 1 && rnd() > n) break; n -= 1;
      const a = rnd() * Math.PI * 2;
      const R = rr(1.6, 2.6);
      const y = rr(0.1, 2.2);
      tmpA.set(c.x + Math.cos(a) * R, y, c.z + Math.sin(a) * R);
      // tangential + inward velocity = spiral
      tmpB.set(-Math.sin(a), 0.12, Math.cos(a)).multiplyScalar(3.2).addScaledVector(tmpC.set(c.x - tmpA.x, 0, c.z - tmpA.z).normalize(), 2.2);
      emberPool.spawn(tmpA.x, tmpA.y, tmpA.z, tmpB.x, tmpB.y, tmpB.z, rr(0.35, 0.6),
        pal.ion.r * HDR(1.6), pal.ion.g * HDR(1.6), pal.ion.b * HDR(1.6));
    }
    chargeLight.position.copy(orbPos);
    chargeLight.color.copy(pal.main);
  ...
  if (st === 'ascend') { // light gathers from above (holy/heals)
    casterChest(orbPos);
    const c = rig.group.position;
    if (rnd() < dt * (14 + p * 30)) {
      emberPool.spawn(c.x + rr(-0.7, 0.7), rr(2.8, 4), c.z + rr(-0.7, 0.7),
        rr(-0.05, 0.05), rr(-2.2, -1.4), rr(-0.05, 0.05), rr(0.7, 1.1),
        pal.ion.r * HDR(1.7), pal.ion.g * HDR(1.7), pal.ion.b * HDR(1.7));
    }
    // held volumetric shaft, brightening with the cast (spec.shaft gates/scales it)
    const ascSc = ctx.spec.shaft === false ? 0 : typeof ctx.spec.shaft === 'number' ? ctx.spec.shaft : 1;
    if (!ctx.shaft && ascSc > 0) ctx.shaft = spawnShaft(new THREE.Vector3(c.x, 0, c.z), { radius: 1.15 * Math.max(0.4, ascSc), height: 7.5 * Math.max(0.5, ascSc), color: pal.glow, dur: Infinity, intensity: 0 });
    if (ctx.shaft) ctx.shaft.mat.uniforms.uIntensity.value = (0.25 + p * 0.85) * Math.min(1, ascSc + 0.3);
  ...
  if (st === 'stance') { // grounded anticipation: dust, pebbles, late hand-glow
    const c = rig.group.position;
    if (rnd() < dt * 10) {
      emberPool.spawn(c.x + rr(-0.5, 0.5), 0.08, c.z + rr(-0.5, 0.5),
        rr(-0.3, 0.3), rr(0.2, 0.6), rr(-0.3, 0.3), rr(0.3, 0.6),
        0.28, 0.26, 0.22); // dusty, not glowing
    }
...
function windupCharge(p, dt, full) {
  ...
  if (breath !== ctx._breathSnd) { ctx._breathSnd = breath; sfx.bedBreath(breath); } // audio holds its breath too
  const orbScale = 0.34 * ctx.power;
  const orbR = orbScale * (0.25 + 0.75 * easeOutCubic(Math.min(p, breath ? 0.86 : 1))) * (1 + Math.sin(stateT * 31) * 0.06 * p * (breath ? 0.15 : 1) + beat);
  ...
  chargeLight.position.copy(orbPos);
  chargeLight.color.copy(pal.main);
  chargeLight.intensity = (3 + easeInCubic(p) * 58) * (breath ? 0.5 : 1) * (1 + beat) * (p > 0.965 ? 1.7 : 1);
  if (full) {
  ...
  let n = rate * dt;
  while (n > 0) { if (n < 1 && rnd() > n) break; n -= 1;
    const a = rnd() * Math.PI * 2, e2 = rr(-0.6, 1.1);
    const R = rr(1.6, 2.8);
    tmpA.set(orbPos.x + Math.cos(a) * Math.cos(e2) * R, orbPos.y + Math.sin(e2) * R, orbPos.z + Math.sin(a) * Math.cos(e2) * R);
    tmpB.copy(orbPos).sub(tmpA).normalize().multiplyScalar(rr(2.6, 4.2));
    emberPool.spawn(tmpA.x, tmpA.y, tmpA.z, tmpB.x, tmpB.y, tmpB.z, R / 3.4, ctx.pal.ion.r * HDR(1.5), ctx.pal.ion.g * HDR(1.5), ctx.pal.ion.b * HDR(1.5));
  }
  if (full) {
    // risers off the circle + micro-arcs + ground arcs
    if (rnd() < dt * 26 * (0.3 + p)) {
      const a = rnd() * Math.PI * 2, R = rr(1.1, 2.05);
      riserPool.spawn(rig.group.position.x + Math.cos(a) * R, 0.1, rig.group.position.z + Math.sin(a) * R,
        rr(-0.1, 0.1), rr(0.5, 1.3), rr(-0.1, 0.1), rr(0.7, 1.4),
        pal.main.r * HDR(1.4), pal.main.g * HDR(1.4), pal.main.b * HDR(1.4));
    }
    if (rnd() < dt * (4 + p * 18)) {
      const p1 = orbPos.clone().add(new THREE.Vector3(rr(-1, 1), rr(-1, 1), rr(-1, 1)).normalize().multiplyScalar(orbR * rr(0.8, 1.4)));
      const p2 = orbPos.clone().add(new THREE.Vector3(rr(-1, 1), rr(-1, 1), rr(-1, 1)).normalize().multiplyScalar(orbR * rr(1.2, 2.6)));
      spawnBoltFx({
        life: rr(0.06, 0.13), flicker: 0.03, width: 0.028, hdr: 2.8,
        colCore: pal.core, colGlow: pal.main,
    ...
    if (groundArcTimer <= 0 && p > 0.25) {
      ...
      const a = rnd() * Math.PI * 2;
      const start = new THREE.Vector3(rig.group.position.x + Math.cos(a) * rr(1.6, 2.05), 0.05, rig.group.position.z + Math.sin(a) * rr(1.6, 2.05));
      const end = rnd() < 0.5 ? orbPos.clone() : new THREE.Vector3(rig.group.position.x + rr(-0.4, 0.4), rr(0.8, 1.6), rig.group.position.z + rr(-0.4, 0.4));
      spawnBoltFx({
        life: rr(0.1, 0.18), flicker: 0.045, width: 0.04, hdr: 2.4,
        colCore: pal.core, colGlow: pal.main,
        gen: () => genBolt(start, end, { jag: 0.24, subdiv: 4, branchP: 0.2, depth: 1 }),
      });
    }
    if (ctx.palName === 'storm') { // storm anticipation is its own weather event
      ...
      if (ctx.ionTimer <= 0 && p > 0.2) {
        ...
        const ia = rnd() * Math.PI * 2, iR = rr(1.2, 2.0);
        const from = new THREE.Vector3(rig.group.position.x + Math.cos(ia) * iR, 0.02, rig.group.position.z + Math.sin(ia) * iR);
        const to = orbPos.clone();
        spawnBoltFx({
          life: 0.09, flicker: 0.04, width: 0.025, hdr: 2.2,
          colCore: pal.ion, colGlow: pal.main,
          gen: () => genBolt(from, to, { jag: 0.22, subdiv: 4, branchP: 0.15, depth: 0 }),
        });
      }
      if (p > 0.9 && !ctx.skyAnswered) { // held breath: the sky answers before the release
        ctx.skyAnswered = true;
        lightPulses.pulse(new THREE.Vector3(rig.group.position.x, 9, rig.group.position.z), pal.ion, 260, 0.4);
...
function releaseFlashFx(scaleMul = 1) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    spawnBoltFx({
      life: 0.11, flicker: 0.035, width: 0.05, hdr: 3,
      colCore: pal.core, colGlow: pal.main,
      gen: () => genBolt(orbPos, end, { jag: 0.22, subdiv: 4, branchP: 0.2, depth: 1 }),
    });
  }
  burst(sparkPool, orbPos.x, orbPos.y, orbPos.z, Math.round(26 * ctx.power), 7, 0.5, pal.ion, HDR(2.2), 0.25);
}
...
function impactStack(pos, o = {}) {
  ...
  if (o.flipbook !== false) spawnFlipbook(pos, 3.2 * cs, 0.5, HDR(1.25 * hdrBoost), cMain, style.sheet);
  if (o.ring !== false) {
    const rs = typeof o.ring === 'number' ? o.ring : 1; // numeric override scales the wavefront
    spawnRing(new THREE.Vector3(pos.x, 0.05, pos.z), 4.6 * cs * rs, 0.55, cMain, HDR(1.7 * hdrBoost));
    setTimeoutSim(0.05, () => spawnRing(new THREE.Vector3(pos.x, 0.05, pos.z), 2.8 * cs * rs, 0.45, pal.accent, HDR(1.3)));
  }
  if (o.vRing !== false) spawnRing(pos.clone(), 3.4 * cs * (typeof o.vRing === 'number' ? o.vRing : 1), 0.4, cIon, HDR(1.2), true);
  if (critFinisher) { // the death-sentence beat: second wave + white flash ring
    setTimeoutSim(0.12, () => {
      spawnRing(new THREE.Vector3(pos.x, 0.05, pos.z), 6 * cs, 0.6, pal.core, HDR(1.6));
      spawnRing(pos.clone(), 4.4 * cs, 0.5, WHITE, HDR(1.4), true);
    });
  }
  burst(sparkPool, pos.x, pos.y, pos.z, Math.round((o.sparks ?? 46) * cs), 11 * cs, 0.7, cIon, HDR(2.4 * hdrBoost), 0.55);
  ...
  if (o.scorch) {
    ...
    const decal = ctx.spec.decal === 'crack' ? 'crack'
      : ctx.spec.decal === 'scorch' ? 'char'
      : ctx.spec.decal === 'rune' ? 'rays'
      : style.decal;
    if (decal === 'char') { scorchDark.material.color.setHex(0x000000); scorchMax = 0.85; }
    else if (decal === 'rime') { scorchDark.material.color.setHex(0xcfe6ff); scorchMax = 0.3; }
    else { scorchMax = 0; }
    scorchDark.position.set(pos.x, 0.025, pos.z);
    crackMesh.position.set(pos.x, 0.035, pos.z);
    crackMat.uniforms.uMap.value = decal === 'rime' ? TEX.shards : decal === 'rays' ? TEX.rays : TEX.cracks;
  ...
  if (style.decal === 'rime' && !o.scorch && !ctx.self && ctx.spec.screenFx !== false) frostEdge(0.7); // screen frost kiss (spec can opt out)
  if (critFinisher) {
    hitstopLeft = 0.09;
  ...
  if (o.receiver === 'caster') { // drains: the energy arrives at the CASTER
  ...
  if (comboOn) {
    ...
    if (lastResiduePal === 'frost' && ctx.palName === 'fire') { // fire flash-melts the rime
      spawnSmoke(pos, 12, 0xe8eef2);
      spawnRing(pos.clone(), 3.6, 0.5, WHITE, HDR(1.3), true);
      screenFlash(0.22);
      scorchDark.material.color.setHex(0x000000); scorchMax = 0.85; // rime → char
      lastResiduePal = null;
      sfx.noise2({ dur: 0.7, freq: 3200, sweep: 500, gain: 0.3, type: 'bandpass', q: 1.2, pan: panOf(pos.x) }); // steam hiss
    }
  }
  // momentary elemental grade shift + anime impact frames on crit finishers
  const G = { flame: [1.18, 0.98, 0.85, 0.5], shatter: [0.85, 0.96, 1.2, 0.5], radiance: [1.15, 1.08, 0.9, 0.45], void: [0.95, 0.88, 1.12, 0.5], verdant: [0.95, 1.1, 0.92, 0.4], electric: [0.95, 1.02, 1.15, 0.4] }[style.sheet];
  ...
  if (o.kick !== false) addKick(projDir, (isCrit && ctx.arch.canCrit ? 1.8 : 1) * 0.16 * (o.shake ?? 1));
  ...
  if (o.crawlArcs) {
    for (let i = 0; i < 6; i++) {
      setTimeoutSim(rr(0, 0.12), () => {
        const a1 = randomOnDummy(), a2 = randomOnDummy();
        spawnBoltFx({
          life: rr(0.14, 0.3), flicker: 0.04, width: 0.045, hdr: 2.6,
          colCore: pal.core, colGlow: pal.main,
    ...
    for (let i = 0; i < 5; i++) {
      const dx = pos.x + rr(-0.9, 0.9), dz = pos.z + rr(-0.9, 0.9);
      emberPool.spawn(dx, 0.15, dz, rr(-0.05, 0.05), rr(0.4, 0.7), rr(-0.05, 0.05), rr(0.7, 1.1),
        pal.ion.r * HDR(1.4), pal.ion.g * HDR(1.4), pal.ion.b * HDR(1.4));
      if (i < 3) {
        setTimeoutSim(rr(0.08, 0.3), () => {
          ...
          spawnBoltFx({
            life: 0.07, flicker: 0.03, width: 0.015, hdr: 2.2,
            colCore: pal.ion, colGlow: pal.main,
...
function spawnTrailArc(style, hp, pal) {
  const mk = (a, b, bow, w = 0.16, up = 0) => {
    ...
    spawnBoltFx({
      life: 0.7, flicker: 0.3, width: w, glowWidth: 0.5, hdr: 2.8,
      colCore: pal.core, colGlow: pal.main,
      curve: (t) => t < 0.06 ? t / 0.06 : Math.pow(1 - (t - 0.06) / 0.94, 1.1), // holds long enough to OWN the money shot
      gen: () => [{ pts, intensity: 1 }],
    });
  };
  ...
  else if (style === 'riposte') mk(V(0.2, -0.95, rr(-0.25, 0.25)), V(-0.45, 1.35, rr(-0.25, 0.25)), 0.45);
  else if (style === 'x') {
...
function spawnBloodFx(pos, cs) {
  ...
  for (const off of [-0.28, 0, 0.28]) {
    ...
    spawnBoltFx({
      life: 1.1, flicker: 0.5, width: 0.07, glowWidth: 0.18, hdr: 1.5,
      colCore: bp.main, colGlow: bp.glow,
      curve: (t) => t < 0.1 ? t / 0.1 : Math.pow(1 - (t - 0.1) / 0.9, 0.7),
      gen: () => [{ pts, intensity: 1 }],
    });
  }
...
function fireFinisher() {
  ...
  spawnBoltFx({
    life: 0.38, flicker: 0.055, width: 0.3, glowWidth: 1.15, hdr: 4.4,
    colCore: pal.core, colGlow: pal.accent,
    curve: (t) => t < 0.12 ? 1 : Math.pow(1 - (t - 0.12) / 0.88, 1.6),
    gen: () => genBolt(top, hp, { jag: 0.09, subdiv: 6, branchP: 0.42, depth: 2, branchScale: 0.3 }),
  });
  spawnRing(new THREE.Vector3(hp.x, 0.06, hp.z), 7, 0.7, pal.accent, HDR(1.5));
  spawnRing(hp.clone(), 5, 0.55, pal.core, HDR(1.2), true);
  spawnShaft(new THREE.Vector3(hp.x, 0, hp.z), { radius: 1, height: 15, color: pal.accent, dur: 0.5, intensity: 1 });
  spawnRipple(hp, 1.4);
  ...
  for (const [delay, w, hd] of [[0.14, 0.18, 3.2], [0.32, 0.1, 2.4]]) {
    setTimeoutSim(delay, () => {
      if (!ctx) return;
      spawnBoltFx({
        life: 0.14, flicker: 0.04, width: w, glowWidth: w * 3.2, hdr: hd,
        colCore: pal.core, colGlow: pal.accent,
...
const ARCHETYPES = {
  bolt: {
    canCrit: true, aftermathDur: 2.2,
    begin() { sfx.chargeStart(ctx.windup / (slowMo ? 0.25 : userTimeScale)); rig.play('Spellcasting', { fade: 0.25 }); },
    windup(p, dt) { windupFx(p, dt); },
    release() {
      rig.play(rig.cls === 'hunter' ? '2H_Ranged_Shoot' : 'Spellcast_Shoot', { loop: false, fade: 0.08, timeScale: 1.35 });
      handMid(orbPos);
      projPos.copy(orbPos);
      projDir.copy(dummy.hitPos).sub(projPos).normalize();
      trailHist.length = 0;
      // per-ability projectile silhouette
      const pStyle = ctx.spec.bolt?.style ?? (ctx.cls === 'hunter' ? 'arrow' : PROJ_DEFAULT[ctx.palName] ?? 'comet');
      clearProjMesh();
      if (pStyle !== 'comet') projMesh = buildProjMesh(pStyle, ctx.pal);
      ctx.projStyle = pStyle;
      ctx.tracerFrom = orbPos.clone();
      releaseFlashFx();
      if (ctx.spec.bolt?.leader) { // real lightning: a dim leader finds the path, the return stroke ANSWERS
        const lFrom = orbPos.clone(), lTo = dummy.hitPos.clone();
        spawnBoltFx({
          life: 0.07, flicker: 0.03, width: 0.035, hdr: 1.6,
          colCore: ctx.pal.ion, colGlow: ctx.pal.main,
          gen: () => genBolt(lFrom, lTo, { jag: 0.18, subdiv: 5, branchP: 0.35, depth: 1 }),
        });
        setTimeoutSim(0.08, () => {
          if (!ctx) return;
          spawnBoltFx({
            life: 0.16, flicker: 0.05, width: 0.22, glowWidth: 1.0, hdr: 4.0,
            colCore: ctx.pal.core, colGlow: ctx.pal.accent,
            curve: (t) => Math.pow(1 - t, 1.4),
      ...
      for (let i = 1; i < volley; i++) {
        setTimeoutSim(i * 0.12, () => {
          if (!ctx || !rig) return;
          const from = handMid(new THREE.Vector3()).add(new THREE.Vector3(0, rr(-0.15, 0.2), rr(-0.3, 0.3)));
          ...
          spawnBoltFx({
            life: 0.22, flicker: 0.05, width: 0.06, glowWidth: 0.2, hdr: 2.6,
            colCore: ctx.pal.core, colGlow: ctx.pal.main,
            curve: (t) => Math.pow(1 - t, 1.1),
            gen: () => [{ pts, intensity: 1 }],
          });
          setTimeoutSim(0.18, () => {
            if (!ctx) return;
      ...
      tick(dt) {
        ...
        if (b.jagged) {
          ...
          if (!ctx.trailCache || ctx.trailFlick <= 0) {
            ...
            ctx.trailCache = genBolt(tail, projPos, { jag: 0.12, subdiv: 4, branchP: 0.3, depth: 1 });
        ...
        updateProjMesh(dt);
        const hs = (b.headScale ?? 1) * ctx.power * (ctx.projStyle === 'arrow' ? 0.35 : ctx.projStyle && ctx.projStyle !== 'comet' ? 0.7 : 1);
        ...
        if (rnd() < dt * 60) {
          sparkPool.spawn(projPos.x, projPos.y, projPos.z,
            rr(-1.5, 1.5), rr(-0.5, 1.5), rr(-1.5, 1.5), rr(0.25, 0.5),
            pal.ion.r * HDR(2), pal.ion.g * HDR(2), pal.ion.b * HDR(2));
        }
        ...
        if (fe > 0) {
          ...
          if (forkTimer <= 0) {
            ...
            spawnBoltFx({
              life: rr(0.08, 0.14), flicker: 0.04, width: 0.09, hdr: 2.2,
              colCore: pal.core, colGlow: pal.main,
    ...
    impact() {
      ...
      ctx.trailCache = null;
      sfx.sizzleStop();
      if (ctx.spec.bolt?.tracer && ctx.tracerFrom) { // the shot's path stays etched in the air
        const tFrom = ctx.tracerFrom.clone(), tTo = dummy.hitPos.clone();
        spawnBoltFx({
          life: 0.9, flicker: 0.35, width: 0.05, glowWidth: 0.16, hdr: 2.4,
          colCore: ctx.pal.core, colGlow: ctx.pal.ion,
          curve: (t) => Math.pow(1 - t, 0.8),
      ...
      if (ctx.spec.finisher && finisherOn) { finisherFired = false; finisherTimer = 0; }
      else finisherFired = true;
    },
    aftermath(p, dt) { standardAftermath(dt, true); },
  },

  burst: {
    canCrit: true, aftermathDur: 1.9,
    begin() { rig.play('Spellcasting', { fade: 0.2 }); },
    windup(p, dt) { windupFx(p, dt); },
    release() {
      const style = ctx.spec.burst?.style ?? 'link';
      rig.play(style === 'skybeam' ? 'Spellcast_Raise' : 'Spellcast_Shoot', { loop: false, fade: 0.08, timeScale: 1.4 });
      handMid(orbPos);
      projDir.copy(dummy.hitPos).sub(orbPos).normalize();
      releaseFlashFx(0.8);
      hideOrbSoon();
      if (style === 'link') { // instantaneous zap link
        const from = orbPos.clone(), to = dummy.hitPos.clone();
        spawnBoltFx({
          life: 0.12, flicker: 0.04, width: 0.06, hdr: 2.6,
          colCore: ctx.pal.core, colGlow: ctx.pal.main,
          gen: () => genBolt(from, to, { jag: 0.08, subdiv: 5, branchP: 0.12, depth: 1 }),
        });
      }
    },
    travel: { tick(dt) { return stateT >= (ctx.spec.burst?.style === 'skybeam' ? 0.22 : 0.12); } },
    impact() {
      const style = ctx.spec.burst?.style ?? 'link';
      const pal = ctx.pal;
      const hp = dummy.hitPos;
      if (style === 'skybeam') {
        ...
        const sbSc = ctx.spec.shaft === false ? 0 : typeof ctx.spec.shaft === 'number' ? ctx.spec.shaft : 1;
        if (sbSc > 0) spawnShaft(new THREE.Vector3(hp.x, 0, hp.z), { radius: 1.35 * sbSc, height: 14, color: pal.main, dur: 0.65, intensity: 1.2 * Math.min(1, sbSc + 0.2) });
        const top = new THREE.Vector3(hp.x, 14, hp.z);
        spawnBoltFx({
          life: 0.5, flicker: 0.12, width: 0.34, glowWidth: 1.5, hdr: 2.6,
          colCore: pal.core, colGlow: pal.main,
          curve: (t) => t < 0.15 ? 1 : Math.pow(1 - (t - 0.15) / 0.85, 1.5),
          gen: () => [{ pts: smoothArc(top, hp.clone(), 0.02, 10, 0), intensity: 1 }],
        });
        spawnBoltFx({
          life: 0.4, flicker: 0.08, width: 0.1, hdr: 3.4,
          colCore: pal.core, colGlow: pal.accent,
        ...
        for (let i = 0; i < 4; i++) {
          ...
          spawnBoltFx({
            life: rr(0.2, 0.32), flicker: 0.05, width: 0.09, hdr: 2.6,
            colCore: pal.core, colGlow: pal.main,
  ...
  strike: {
    melee: true, canCrit: true, aftermathDur: 1.6,
    begin() { rig.play('Idle'); },
    windup(p, dt) { windupFx(p, dt); },
    release() {
    ...
    travel: { tick(dt) { return stateT >= (ctx.contactAt ?? 0.2); } }, // blade-contact frame
    impact() {
      const pal = ctx.pal;
      const hp = dummy.hitPos;
      ...
      if (arc === 'vertical') {          // overhead chop
        ...
        for (const off of [-0.3, 0, 0.3]) {
          ...
          spawnBoltFx({
            life: 0.22, flicker: 0.11, width: 0.11, glowWidth: 0.34, hdr: 2.6,
            colCore: pal.core, colGlow: pal.main,
            curve: (t) => Math.pow(1 - t, 1.3),
            gen: () => [{ pts, intensity: 1 }],
          });
        }
      } else if (arc === 'bite') {       // jaws snapping shut
        const top = smoothArc(hp.clone().add(new THREE.Vector3(-0.75, 1.15, 0)), hp.clone().add(new THREE.Vector3(0.65, 0.15, 0)), 0.45, 10, 0);
        const bot = smoothArc(hp.clone().add(new THREE.Vector3(-0.75, -0.95, 0)), hp.clone().add(new THREE.Vector3(0.65, -0.05, 0)), -0.45, 10, 0);
        for (const pts of [top, bot]) {
          spawnBoltFx({
            life: 0.2, flicker: 0.1, width: 0.15, glowWidth: 0.42, hdr: 2.7,
            colCore: pal.core, colGlow: pal.main,
            curve: (t) => Math.pow(1 - t, 1.5),
            gen: () => [{ pts, intensity: 1 }],
          });
        }
      ...
      if (a) { // single-ribbon arcs (claws/bite draw their own)
        const arcPts = smoothArc(a, b, bow, 14, up);
        spawnBoltFx({
          life: 0.38, flicker: 0.16, width: arc === 'thrust' ? 0.1 : 0.16, glowWidth: 0.5, hdr: 2.8,
          colCore: pal.core, colGlow: pal.main,
          curve: (t) => Math.pow(1 - t, 1.2),
          gen: () => [{ pts: arcPts, intensity: 1 }],
        });
      }
      rig.hold(0.11); // body bites on contact
      impactStack(hp, { scale: 0.8, flipbook: true, ring: !!st.groundSlam, vRing: false, sparks: 26, scorch: !!st.groundSlam, smoke: false });
      if (st.groundSlam) {
        trauma = Math.max(trauma, 0.55);
        spawnRing(new THREE.Vector3(hp.x, 0.05, hp.z), 4.2 * ctx.power, 0.5, pal.main, HDR(1.6));
      }
      if (st.stars || ctx.spec.cc?.style === 'stars') spawnOrbitStars(new THREE.Vector3(hp.x, 2.55, hp.z), pal, 1.4);
      if (st.bleed) startBurn(pal, 2.6, 'fall'); // wound keeps weeping
      const swings = st.swings ?? 1;
      if (swings > 1) {
        setTimeoutSim(0.22, () => {
          rig.play(rig.attacks[Math.floor(rnd() * rig.attacks.length)], { loop: false, fade: 0.05, timeScale: 1.4 });
          setTimeoutSim(0.16, () => impactStack(hp, { scale: 0.7, ring: false, vRing: false, sparks: 20, scorch: false, smoke: false }));
        });
      }
    },
    aftermath(p, dt) { standardAftermath(dt, false); },
  ...
  nova: {
    canCrit: true, aftermathDur: 2.0, centered: true,
    begin() { rig.play('Spellcasting', { fade: 0.15 }); },
    windup(p, dt) { windupFx(p, dt); },
    release() {
    ...
    travel: { tick(dt) { return stateT >= 0.1; } },
    impact() {
      const pal = ctx.pal;
      ...
      setTimeoutSim(0.06, () => spawnRing(center.clone(), radius * 0.65, 0.5, pal.accent, HDR(1.4)));
      ...
      for (let i = 0; i < 8; i++) {
        ...
        const to = new THREE.Vector3(c.x + Math.cos(a) * radius * rr(0.7, 1), 0.05, c.z + Math.sin(a) * radius * rr(0.7, 1));
        spawnBoltFx({
          life: rr(0.14, 0.24), flicker: 0.05, width: 0.07, hdr: 2.4,
          colCore: pal.core, colGlow: pal.main,
      ...
      // ring of risers
      for (let i = 0; i < 22; i++) {
        const a = rnd() * Math.PI * 2, R = rr(0.5, radius * 0.9);
        riserPool.spawn(c.x + Math.cos(a) * R, 0.15, c.z + Math.sin(a) * R,
          rr(-0.1, 0.1), rr(1, 2.4), rr(-0.1, 0.1), rr(0.5, 1),
          pal.ion.r * HDR(1.8), pal.ion.g * HDR(1.8), pal.ion.b * HDR(1.8));
      }
      ...
      crackMat.uniforms.uMap.value = nStyle.decal === 'rime' ? TEX.shards : nStyle.decal === 'rays' ? TEX.rays : TEX.cracks;
      ...
      const critFinisher = critHit && (ctx.spec.finisher || ctx.power >= 1.3);
      sfx.impact(ctx.palName, { pan: panOf(c.x), scale: ctx.power * (critHit ? 1.35 : 1), crit: critHit, finisher: critFinisher });
  ...
  beam: {
    canCrit: false, aftermathDur: 1.4,
    begin() { rig.play('Spellcasting', { fade: 0.2 }); ctx.beamTicks = 0; ctx.beamPulses = []; },
    windup(p, dt) { windupFx(p, dt); },
    release() {
      handMid(orbPos);
      projDir.copy(dummy.hitPos).sub(orbPos).normalize();
    ...
    channelTick(dt) {
      ...
      if (!ctx.beamCache || ctx.beamFlick <= 0) { // flicker cache: regen ~22x/s, not every frame
        ctx.beamFlick = 0.045;
        ctx.beamCache = genBolt(from, to, { jag: 0.035, subdiv: 4, branchP: 0.06, depth: 1 });
      }
      for (const line of ctx.beamCache) {
        ribbons.add(line.pts, 0.5 * grow * line.intensity, pal.glow, HDR(1.1 * grow) * line.intensity, 0.7, fdir);
        ribbons.add(line.pts, 0.16 * grow * line.intensity, pal.main, HDR(1.8) * line.intensity, 0.7, fdir);
        ribbons.add(line.pts, 0.06 * grow * line.intensity, pal.core, HDR(3.0) * line.intensity, 0.7, fdir * 1.4);
      }
      // traveling pulses
      ...
      for (let i = ctx.beamPulses.length - 1; i >= 0; i--) {
        ...
        if (u < 0 || u > 1) { ctx.beamPulses.splice(i, 1); continue; }
        tmpA.copy(from).lerp(to, u);
        emberPool.spawn(tmpA.x, tmpA.y, tmpA.z, 0, 0, 0, 0.14, pal.core.r * HDR(2.4), pal.core.g * HDR(2.4), pal.core.b * HDR(2.4));
      }
      chargeLight.position.copy(orbPos);
      chargeLight.color.copy(pal.main);
      ...
      if (stateT >= tickAt && ctx.beamTicks < ticks) {
        ...
        impactStack(b.drain ? casterChest(new THREE.Vector3()) : dummy.hitPos, { scale: last ? 0.95 : 0.42 + 0.13 * ctx.beamTicks, ring: last, vRing: false, flipbook: last, sparks: 14 + ctx.beamTicks * 4, scorch: false, smoke: false, hitstop: false, kick: false, shake: last ? 0.8 : 0.4, liteAudio: !last, receiver: b.drain ? 'caster' : undefined, shell: b.drain ? false : undefined });
        if (b.drain) { // the victim visibly pays for what the caster gains
    ...
    impact() { hideOrb(); sfx.beamStop(); projLight.intensity = 0; },
    aftermath(p, dt) { standardAftermath(dt, false); },
  },

  dot: {
    canCrit: false, aftermathDur: 3.6,
    begin() { rig.play('Spellcasting', { fade: 0.2 }); },
    windup(p, dt) { windupFx(p, dt); },
    release() {
      ...
      const from = orbPos.clone(), to = dummy.hitPos.clone();
      spawnBoltFx({
        life: 0.16, flicker: 0.05, width: 0.05, hdr: 2.2,
        colCore: ctx.pal.core, colGlow: ctx.pal.main,
        gen: () => [{ pts: smoothArc(from, to, rr(-0.8, 0.8), 18, 0.6), intensity: 1 }],
      });
    },
    travel: { tick(dt) { return stateT >= 0.16; } },
    impact() {
  ...
  heal: {
    canCrit: false, aftermathDur: 1.8, self: true,
    begin() { rig.play('Spellcasting', { fade: 0.25 }); sfx.chargeStart(ctx.windup / (slowMo ? 0.25 : userTimeScale)); },
    windup(p, dt) { windupFx(p, dt); },
    release() {
    ...
    impact() {
      ...
      const shaftSc = ctx.spec.shaft === false ? 0 : typeof ctx.spec.shaft === 'number' ? ctx.spec.shaft : 1;
      if (ctx.windupStyle !== 'none' && shaftSc > 0) {
        spawnShaft(new THREE.Vector3(c.x, 0, c.z), {
          radius: (0.55 + 0.45 * ctx.power) * shaftSc, height: (4 + 3.2 * ctx.power) * shaftSc,
          color: pal.main, dur: 0.5 + 0.4 * ctx.power, intensity: (0.45 + 0.45 * ctx.power) * Math.min(1, shaftSc + 0.25),
        });
      }
      setRigGlow();
      spawnRing(new THREE.Vector3(c.x, 0.05, c.z), 2.6 * ctx.power, 0.7, pal.main, HDR(1.3));
      for (let i = 0; i < 26; i++) {
        const a = rnd() * Math.PI * 2, R = rr(0.2, 1);
        riserPool.spawn(c.x + Math.cos(a) * R, rr(0.1, 1.6), c.z + Math.sin(a) * R,
          rr(-0.08, 0.08), rr(0.7, 1.5), rr(-0.08, 0.08), rr(0.9, 1.6),
          pal.ion.r * HDR(1.9), pal.ion.g * HDR(1.9), pal.ion.b * HDR(1.9));
      }
  ...
  buff: {
    canCrit: false, aftermathDur: 3.2, self: true,
    begin() { rig.play('Idle'); },
    windup(p, dt) { windupFx(p, dt); },
    release() {
      ...
      if (style === 'morph') {
        ...
        for (let i = 0; i < 22; i++) {
          const a = rnd() * Math.PI * 2, e = rr(-0.6, 1.1), R = rr(1.2, 2);
          tmpA.set(c.x + Math.cos(a) * Math.cos(e) * R, c.y + Math.sin(e) * R, c.z + Math.sin(a) * Math.cos(e) * R);
          tmpB.copy(c).sub(tmpA).normalize().multiplyScalar(rr(3.5, 5.5));
          emberPool.spawn(tmpA.x, tmpA.y, tmpA.z, tmpB.x, tmpB.y, tmpB.z, R / 4.5, ctx.pal.ion.r * HDR(1.8), ctx.pal.ion.g * HDR(1.8), ctx.pal.ion.b * HDR(1.8));
        }
      }
    },
    travel: { tick(dt) { return stateT >= 0.12; } },
    impact() {
      const style = ctx.spec.buff?.style ?? 'raise';
      const pal = ctx.pal;
      const c = casterChest(new THREE.Vector3());
      ...
      const orbit = ctx.spec.buff?.orbit ?? (pal === PALETTES.storm ? 'sparks' : style === 'raise' ? 'runes' : 'none');
    ...
    aftermath(p, dt) {
      ...
      if (ctx.buffHold && stateT < ctx.buffHold) {
        ...
        casterGlowV = Math.max(casterGlowV, (0.42 + Math.sin(perfNow * 4) * 0.08) * Math.min(1, (ctx.buffHold - stateT) / 0.6));
      }
      if (ctx.runeFlash) {
        runeDissolve = Math.min(1, runeDissolve + dt * 2.2);
        runeMat.uniforms.uDissolve.value = runeDissolve;
        if (runeDissolve >= 1) { runeMesh.visible = false; ctx.runeFlash = false; }
      }
    },
  },

  shout: {
    canCrit: false, aftermathDur: 1.6, self: true, centered: true,
    begin() { rig.play('Idle'); },
    windup(p, dt) { windupFx(p, dt); },
    release() {
    ...
    travel: { tick(dt) { return stateT >= 0.22; } },
    impact() {
      const pal = ctx.pal;
      ...
      setTimeoutSim(0.08, () => spawnRing(new THREE.Vector3(c.x, 0.05, c.z), radius * 1.25, 0.6, pal.accent, HDR(1.8 * rs)));
      setTimeoutSim(0.16, () => spawnRing(c.clone(), radius * 1.1, 0.55, pal.ion, HDR(1.4 * rs), true));
      spawnRipple(c, 1.2); // the air itself bends with the cry
      for (let i = 0; i < 8; i++) {
        const a = i / 8 * Math.PI * 2;
        const from = c.clone();
        const to = new THREE.Vector3(c.x + Math.cos(a) * radius * 0.8, c.y + rr(-0.6, 0.8), c.z + Math.sin(a) * radius * 0.8);
        spawnBoltFx({
          life: 0.2, flicker: 0.07, width: 0.09, hdr: 2.2,
          colCore: pal.core, colGlow: pal.main,
          gen: () => [{ pts: smoothArc(from, to, rr(-0.5, 0.5), 12, rr(0, 0.5)), intensity: 1 }],
        });
      }
      for (let i = 0; i < 14; i++) { // the ground answers: kicked dust ring
        const a = rnd() * Math.PI * 2, R = rr(0.8, radius * 0.45);
        emberPool.spawn(c.x + Math.cos(a) * R, 0.1, c.z + Math.sin(a) * R,
          Math.cos(a) * rr(0.6, 1.6), rr(0.3, 0.9), Math.sin(a) * rr(0.6, 1.6), rr(0.4, 0.8),
          0.3, 0.28, 0.24);
      }
      casterGlowV = 1.6;
      casterShellMat.uniforms.uColor.value.copy(pal.main);
      ...
      if (ctx.spec.shout?.target) { // taunts: the VICTIM visibly hears it
  ...
  summon: {
    canCrit: false, aftermathDur: 2.2, self: true,
    begin() { rig.play('Spellcasting', { fade: 0.25 }); sfx.chargeStart(ctx.windup / (slowMo ? 0.25 : userTimeScale)); },
    windup(p, dt) { windupFx(p, dt); },
    release() {
    ...
    travel: { tick(dt) { return stateT >= 0.15; } },
    impact() {
      const pal = ctx.pal;
      ...
      for (let i = 0; i < 3; i++) {
        ...
        spawnBoltFx({
          life: 0.5, flicker: 0.06, width: 0.09, hdr: 2.2,
          colCore: pal.core, colGlow: pal.main,
          curve: (t) => Math.pow(1 - t, 1.1),
      ...
      for (let i = 0; i < 20; i++) {
        const a = rnd() * Math.PI * 2, R = rr(0.2, 1.1);
        riserPool.spawn(spot.x + Math.cos(a) * R, 0.1, spot.z + Math.sin(a) * R,
          rr(-0.05, 0.05), rr(1, 2.2), rr(-0.05, 0.05), rr(0.7, 1.3),
          pal.ion.r * HDR(1.9), pal.ion.g * HDR(1.9), pal.ion.b * HDR(1.9));
      }
      ...
      else { sfx.heal(ctx.palName === 'nature' ? 'nature' : 'holy', panOf(spot.x)); sfx.buffCast('raise', ctx.palName, panOf(spot.x)); }
    },
    aftermath(p, dt) {
      standardAftermath(dt, false);
  ...
  cc: {
    canCrit: false, aftermathDur: 2.0,
    begin() { rig.play('Spellcasting', { fade: 0.2 }); if (ctx.windup > 0.8) sfx.chargeStart(ctx.windup / (slowMo ? 0.25 : userTimeScale)); },
    windup(p, dt) { windupFx(p, dt); },
    release() {
      ...
      const from = orbPos.clone(), to = dummy.hitPos.clone();
      spawnBoltFx({
        life: 0.14, flicker: 0.05, width: 0.05, hdr: 2.2,
        colCore: ctx.pal.core, colGlow: ctx.pal.main,
    ...
    impact() {
      ...
      if (style === 'poof') {
        ...
        spawnOrbitStars(new THREE.Vector3(hp.x, 2.55, hp.z), pal, Math.max(1.8, (ctx.spec.linger ?? 0) - 0.2));
        screenFlash(0.14);
        sfx.ccPoof(panOf(hp.x));
      } else if (style === 'tendrils') {
        for (let i = 0; i < 7; i++) {
          const a1 = randomOnDummy(), a2 = randomOnDummy();
          setTimeoutSim(rr(0, 0.2), () => spawnBoltFx({
            life: rr(0.2, 0.4), flicker: 0.05, width: 0.05, hdr: 2.2,
            colCore: pal.core, colGlow: pal.main,
        ...
        spawnOrbitStars(new THREE.Vector3(hp.x, 2.55, hp.z), pal, Math.max(1.8, (ctx.spec.linger ?? 0) - 0.2));
        impactStack(hp, { scale: 0.55, ring: false, vRing: false, flipbook: false, sparks: 16, scorch: false, smoke: false });
      }
  ...
  dash: {
    canCrit: true, aftermathDur: 1.7,
    begin() { rig.play('Idle'); ctx.dashTrail = []; },
    windup(p, dt) { windupFx(p, dt); },
    release() {
    ...
    travel: {
      tick(dt) {
        ...
        if (ctx.dashTrail.length > 3) ribbons.add(ctx.dashTrail, 0.7, ctx.pal.main, HDR(1.2), 0.9, 0.8);
        if (rnd() < dt * 40) {
          emberPool.spawn(rig.group.position.x + rr(-0.3, 0.3), 0.15, rig.group.position.z + rr(-0.3, 0.3),
            rr(-0.4, 0.4), rr(0.3, 1), rr(-0.4, 0.4), rr(0.3, 0.6),
            ctx.pal.ion.r, ctx.pal.ion.g, ctx.pal.ion.b);
        }
    ...
    aftermath(p, dt) { standardAftermath(dt, false); if (p > 0.6) rig.play('Idle', { fade: 0.3 }); },
  },
};
...
function hideOrbSoon() { setTimeoutSim(0.1, hideOrb); }
function windupHandGlow(p, dt) {
  handMid(orbPos);
  orbVisible(true);
  const pal = ctx.pal;
  orbShell.visible = false;
...
function spawnOrbitStarsSoft(c, pal) {
  for (let i = 0; i < 10; i++) {
    const a = rnd() * Math.PI * 2, R = rr(0.5, 0.9);
    riserPool.spawn(c.x + Math.cos(a) * R, c.y + rr(-1, 0.4), c.z + Math.sin(a) * R,
      Math.cos(a + 1.7) * 0.6, rr(0.3, 0.8), Math.sin(a + 1.7) * 0.6, rr(0.8, 1.4),
      pal.ion.r * HDR(1.6), pal.ion.g * HDR(1.6), pal.ion.b * HDR(1.6));
  }
}
...
function standardAftermath(dt, groundBurn) {
  ...
  if (runeMesh.visible && !ctx.runeFlash) { // cast circle burns out gracefully
  ...
  if (stateT >= 0.55 && !idleResumed) { idleResumed = true; rig.play('Idle', { fade: 0.4 }); }
  // finisher (only when the spec asks)
  if (ctx.spec.finisher && finisherOn && !finisherFired) {
    finisherTimer += dt;
    if (finisherTimer >= 0.16) { finisherFired = true; fireFinisher(); }
  }
}
...
function setState(s) {
  state = s; stateT = 0;
  if (cineOn) { // trailer language: wide on the windup, punch in for the hit
...
const MOTIFS = {
  fissure: { phase: 'impact', run(at) { // ground splits toward the target
    const pal = ctx.pal;
    const from = new THREE.Vector3(rig.group.position.x + 1, 0.04, rig.group.position.z);
    const to = new THREE.Vector3(at.x, 0.04, at.z);
    spawnBoltFx({
      life: 0.8, flicker: 0.2, width: 0.16, glowWidth: 0.5, hdr: 2.2,
      colCore: pal.core, colGlow: pal.main,
      curve: (t) => t < 0.2 ? t / 0.2 : Math.pow(1 - (t - 0.2) / 0.8, 1.2),
      gen: () => genBolt(from, to, { jag: 0.06, subdiv: 5, branchP: 0.3, depth: 1, branchScale: 0.2 }),
    });
    // dust erupting along the crack
    for (let i = 0; i < 6; i++) {
      const u = (i + 1) / 7;
      const p = from.clone().lerp(to, u);
      setTimeoutSim(u * 0.22, () => burst(emberPool, p.x, 0.1, p.z, 5, 2.2, 0.6, ctx.pal.ion, HDR(1.5), 1));
    }
    trauma = Math.max(trauma, 0.3);
  } },
  vines: { phase: 'impact', run(at) { // living coils climb the target
    const pal = ctx.pal;
    for (let i = 0; i < 5; i++) {
      ...
      spawnBoltFx({
        life: rr(1.1, 1.5), flicker: 0.5, width: 0.09, glowWidth: 0.26, hdr: 1.8,
        colCore: pal.ion, colGlow: pal.main,
        curve: (t) => t < 0.25 ? t / 0.25 : Math.pow(1 - (t - 0.25) / 0.75, 0.8),
        gen: () => [{ pts, intensity: 1 }],
      });
    }
    burst(riserPool, at.x, 0.2, at.z, 12, 1.6, 1.2, ctx.pal.ion, HDR(1.6), 1);
  } },
  chains: { phase: 'impact', run(at) { // taut binding chains snap up from the earth
    const pal = ctx.pal;
    for (let i = 0; i < 3; i++) {
      const a = i / 3 * Math.PI * 2 + 0.5;
      const anchor = new THREE.Vector3(at.x + Math.cos(a) * 2.2, 0.03, at.z + Math.sin(a) * 2.2);
      const grip = new THREE.Vector3(at.x + Math.cos(a) * 0.25, rr(1.2, 1.9), at.z + Math.sin(a) * 0.25);
      ...
      spawnBoltFx({
        life: 0.9, flicker: 0.45, width: 0.07, glowWidth: 0.2, hdr: 2.0,
        colCore: pal.core, colGlow: pal.accent,
        curve: (t) => t < 0.12 ? t / 0.12 : Math.pow(1 - (t - 0.12) / 0.88, 0.9),
        gen: () => [{ pts, intensity: 1 }],
      });
      lightPulses.pulse(anchor, pal.accent, 50, 0.3);
    }
  } },
  swarm: { phase: 'impact', run(at) { // dark shapes burst out and flee
    const pal = ctx.pal;
    for (let i = 0; i < 14; i++) {
      const a = rnd() * Math.PI * 2;
      emberPool.spawn(at.x, at.y ?? 1.5, at.z,
        Math.cos(a) * rr(2, 5), rr(1, 3.2), Math.sin(a) * rr(2, 5), rr(0.7, 1.2),
        pal.glow.r * HDR(1.4), pal.glow.g * HDR(1.4), pal.glow.b * HDR(1.4));
    }
  } },
  pillars: { phase: 'impact', run(at) { // ring of sequential ground eruptions
    const pal = ctx.pal;
    for (let i = 0; i < 5; i++) {
      ...
      setTimeoutSim(i * 0.09, () => {
        ...
        spawnBoltFx({
          life: 0.28, flicker: 0.06, width: 0.12, hdr: 2.4,
          colCore: ctx.pal.core, colGlow: ctx.pal.main,
  ...
  orbitals: { phase: 'impact', run(at) { // orbs circle in, then slam one-two-three
    const pal = ctx.pal;
    for (let i = 0; i < 3; i++) {
      setTimeoutSim(0.1 + i * 0.14, () => {
        const a = i / 3 * Math.PI * 2 + perfNow;
        const from = new THREE.Vector3(at.x + Math.cos(a) * 1.8, (at.y ?? 1.5) + 0.8, at.z + Math.sin(a) * 1.8);
        spawnBoltFx({
          life: 0.14, flicker: 0.05, width: 0.09, hdr: 2.8,
          colCore: pal.core, colGlow: pal.accent,
          gen: () => [{ pts: smoothArc(from, new THREE.Vector3(at.x, at.y ?? 1.5, at.z), 0.4, 10, 0.2), intensity: 1 }],
        });
  ...
  fountain: { phase: 'impact', run(at) { // arcing streams rain onto the point
    const pal = ctx.pal;
    for (let i = 0; i < 4; i++) {
      const a = i / 4 * Math.PI * 2 + 0.4;
      const from = new THREE.Vector3(at.x + Math.cos(a) * 2.4, 0.4, at.z + Math.sin(a) * 2.4);
      spawnBoltFx({
        life: 0.7, flicker: 0.16, width: 0.08, glowWidth: 0.24, hdr: 1.8,
        colCore: pal.ion, colGlow: pal.main,
        curve: (t) => Math.sin(t * Math.PI),
        gen: () => [{ pts: smoothArc(from, new THREE.Vector3(at.x, 0.6, at.z), rr(-0.3, 0.3), 12, 2.2), intensity: 1 }],
      });
    }
  } },
  crescents: { phase: 'impact', run(at) { // twin moon-blades sweep through
    const pal = ctx.pal;
    for (const side of [-1, 1]) {
      ...
      spawnBoltFx({
        life: 0.3, flicker: 0.15, width: 0.2, glowWidth: 0.6, hdr: 2.4,
        colCore: pal.core, colGlow: pal.accent,
        curve: (t) => Math.pow(1 - t, 1.3),
  ...
  bladestorm: { phase: 'release', run(at) { // slash ribbons whirl around the caster
    const pal = ctx.pal;
    const c = casterChest(new THREE.Vector3());
    for (let i = 0; i < 3; i++) {
      const ph = i / 3 * Math.PI * 2;
      spawnBoltFx({
        life: 0.55, flicker: 0.03, width: 0.11, glowWidth: 0.3, hdr: 2.2,
        colCore: pal.core, colGlow: pal.main,
        curve: (t) => Math.pow(1 - t, 0.8),
        gen: () => {
          ...
          for (let s = 0; s <= 8; s++) {
            const a = t0 + s / 8 * 1.6;
            pts.push(new THREE.Vector3(c.x + Math.cos(a) * 1.1, c.y + Math.sin(t0 * 0.7) * 0.3, c.z + Math.sin(a) * 1.1));
          }
  ...
  implosion: { phase: 'release', run(at) { // space sucks inward before the hit
    const pal = ctx.pal;
    const c = new THREE.Vector3(at.x, at.y ?? 1.5, at.z);
    for (let i = 0; i < 18; i++) {
      const a = rnd() * Math.PI * 2, e = rr(-0.5, 1), R = rr(1.4, 2.4);
      tmpA.set(c.x + Math.cos(a) * Math.cos(e) * R, c.y + Math.sin(e) * R, c.z + Math.sin(a) * Math.cos(e) * R);
      tmpB.copy(c).sub(tmpA).normalize().multiplyScalar(rr(4, 6));
      emberPool.spawn(tmpA.x, tmpA.y, tmpA.z, tmpB.x, tmpB.y, tmpB.z, R / 5,
        pal.glow.r * HDR(1.6), pal.glow.g * HDR(1.6), pal.glow.b * HDR(1.6));
    }
  } },
  gavel: { phase: 'impact', run(at) { // a hammer of light descends and LANDS
    const pal = ctx.pal;
    ...
    const coreM = new THREE.MeshBasicMaterial({ color: pal.main.clone().multiplyScalar(0.22), transparent: true, opacity: 0.95 });
    const rimM = new THREE.MeshBasicMaterial({ color: pal.core.clone().multiplyScalar(HDR(2.2)), transparent: true, opacity: 0.85, blending: THREE.AdditiveBlending, depthWrite: false, toneMapped: false, side: THREE.BackSide });
    ...
    setTimeoutSim(1.25, () => {
      scene.remove(grp);
      coreM.dispose(); rimM.dispose();
      head.geometry.dispose(); headRim.geometry.dispose(); handle.geometry.dispose();
    }, true); // sticky: the cleanup must run even if the ability switches mid-swing
  } },
  claws: { phase: 'impact', run(at) { // three raking gashes across the anchor
    const pal = ctx.pal;
    for (const off of [-0.3, 0, 0.3]) {
      ...
      const pts = smoothArc(ca, cb, 0.25, 10, 0);
      spawnBoltFx({
        life: 0.3, flicker: 0.14, width: 0.11, glowWidth: 0.34, hdr: 2.6,
        colCore: pal.core, colGlow: pal.main,
        curve: (t) => Math.pow(1 - t, 1.3),
        gen: () => [{ pts, intensity: 1 }],
      });
  ...
  barrier: { phase: 'impact', run(at) { // plates snap into a guarding arc
    const pal = ctx.pal;
    const c = rig.group.position;
    for (let i = 0; i < 5; i++) {
      const a = (i - 2) * 0.35;
      const m = new THREE.Mesh(new THREE.PlaneGeometry(0.42, 0.6),
        new THREE.MeshBasicMaterial({ color: pal.main.clone().multiplyScalar(HDR(1.8)), transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide, toneMapped: false }));
      m.position.set(c.x + Math.cos(a) * 1.15, 1.35 + Math.abs(i - 2) * 0.1, c.z + Math.sin(a) * 1.15);
      m.lookAt(c.x, 1.35, c.z);
      scene.add(m);
      const born = perfNow;
      shockRings.push({ // reuse the ring updater lifecycle via a shim
        m, mat: { uniforms: { uProgress: { value: 0 } }, dispose() { m.material.dispose(); } },
        age: -i * 0.05, dur: 1.1,
      });
      m.onBeforeRender = () => { // simple envelope, no shader needed
        const t = clamp01((perfNow - born + i * 0.05) / 1.1);
        m.material.opacity = t < 0.15 ? t / 0.15 : Math.pow(1 - (t - 0.15) / 0.85, 1.4) * 0.75;
      };
    }
  } },
};
...
function doImpact() {
  ...
  const anchor = ctx.spec.motifAt === 'target' ? dummy.hitPos.clone()
    : ctx.spec.motifAt === 'caster' ? casterChest(new THREE.Vector3())
    : (ctx.self || ctx.arch.centered) ? casterChest(new THREE.Vector3())
    : dummy.hitPos.clone();
  runMotifs('impact', anchor);
  const spr = ctx.spec.spirit;
  if (spr && (spr.at ?? 'caster') !== 'caster') {
    const at = spr.at === 'portal' && ctx.portalSpot
    ...
    spawnSpirit({ ...spr, scale: (spr.scale ?? 1) * critScale, at, dirX: 1, atKind: spr.at ?? 'target' });
  }
}

function resetTransients(soft = false) {
  if (!soft) boltFx.length = 0; // soft (combo) keeps self-expiring residues alive across beats
  sfx.chargeStop(); sfx.sizzleStop(); sfx.beamStop();
  clearShafts();
  clearProjMesh();
  heatSrc = null;
  if (ctx) ctx.shaft = null;
  if (!soft) { // combo steps keep burns, buffs, footprints, ground wear alive
    ...
    for (const o of orbiters) for (const s of o.sprites) { scene.remove(s); s.material.dispose(); }
    orbiters.length = 0;
  }
  hideOrb();
  ...
  runeMesh.visible = false;
  portalMesh.visible = false;
  runeDissolve = 1; portalDissolve = 1;
  projLight.intensity = 0;
  if (!soft) { shellGlow = 0; targetGlow = 0; casterGlowV = 0; }
  finisherFired = true; finisherTimer = 0;
  idleResumed = true;
}
...
function startLoopForAbility(soft = false) {
  ...
  ctx.novaCenter = beat?.at === 'target' && ctx.arch.centered ? new THREE.Vector3(TARGET_POS.x, 0, TARGET_POS.z) : null;
  if (comboOn) {
...
function advanceAbility(delta) {
  ...
  if (ai >= list.length) { ci = (ci + 1) % classes.length; ai = 0; }
  if (ai < 0) { ci = (ci - 1 + classes.length) % classes.length; ai = CATALOG[classes[ci]].length - 1; }
...
function loopEnd() {
  ...
  if (tourOn) { advanceAbility(1); return; }
  startLoopForAbility();
}
...
function syncSelectors() {
  if (selClass.value !== ctx.cls) { selClass.value = ctx.cls; fillAbilities(ctx.cls); }
  selAbility.value = ctx.id;
}
selClass.addEventListener('change', () => { fillAbilities(selClass.value); setAbility(selClass.value, CATALOG[selClass.value][0].id); });
...
btnTour.addEventListener('click', () => { tourOn = !tourOn; btnTour.classList.toggle('on', tourOn); });
const btnCombo = document.getElementById('btnCombo');
btnCombo.addEventListener('click', () => {
  comboOn = !comboOn;
  btnCombo.classList.toggle('on', comboOn);
  if (comboOn && ctx) {
    tourOn = false; btnTour.classList.remove('on');
    comboIx = 0;
    isCrit = false;
...
speedSlider.addEventListener('input', () => { userTimeScale = parseFloat(speedSlider.value); });
...
btnRec.addEventListener('click', () => {
  ...
  try {
    ...
    setTimeout(() => { if (recorder && recorder.state !== 'inactive') recorder.stop(); }, comboOn ? 16000 : 6000); // combos record the whole chain
  } catch (e) {
...
function updateAbBtn() { // appears only once a generated pack is actually loaded
...
btnAB.addEventListener('click', () => { sfx.sampleMode = !sfx.sampleMode; updateAbBtn(); });
const btnSnd = document.getElementById('btnSound');
btnSnd.addEventListener('click', () => {
  sndOn = !sndOn;
  if (sndOn) { sfx.enable(); sfx.ambienceStart(); }
  else { sfx.chargeStop(); sfx.sizzleStop(); sfx.beamStop(); sfx.ambienceStop(); }
...
function frame(nowMs) {
  ...
  if (ctx && rig) {
    if (state === 'idle') {
      ...
      if (stateT >= ctx.windup) {
        setState('release');
        ctx.arch.release();
        runMotifs('release', ctx.self || ctx.arch.self ? casterChest(new THREE.Vector3()) : dummy.hitPos.clone());
        bodyLeanVel += 0.9; // release recoil snaps the body back
        setTimeoutSim(0.15, hideOrb); // no windup style leaves a stale glow
        if (ctx.shaft) { ctx.shaft.end(); ctx.shaft = null; }
        const spr = ctx.spec.spirit;
        if (spr && (spr.at ?? 'caster') === 'caster') {
          const critScale = isCrit && ctx.arch.canCrit ? 1.25 : 1;
          spawnSpirit({
            ...spr, scale: (spr.scale ?? 1) * critScale,
            at: new THREE.Vector3(rig.group.position.x, 0, rig.group.position.z), dirX: 1,
            atKind: 'caster', follow: ctx.spec.archetype === 'dash', // dash spirits run WITH the caster
          });
        }
      }
      ...
      if (stateT >= 0.1) {
        ...
        else { setState('aftermath'); idleResumed = false; doImpact(); }
      }
  ...
  if (releaseFlash.visible) {
    ...
    releaseFlash.scale.setScalar((2.2 + easeOutCubic(rp) * 3.4) * (releaseFlash.userData.scale ?? 1));
    releaseFlash.material.opacity = Math.pow(1 - rp, 1.2);
  ...
  if (impactFlash.visible) {
    ...
    impactFlash.material.opacity = Math.pow(1 - ip, 1.5) * 0.85;
  ...
  for (const m of dummy.mats) m.emissiveIntensity = Math.max(targetGlow * 0.28, comboOn ? comboWear * 0.35 : 0);
  dummy.shellMat.uniforms.uIntensity.value = shellGlow;
  dummy.shellMat.uniforms.uTime.value = perfNow % 3600;
  dummy.shell.visible = shellGlow > 0.02 && !!(ctx && ctx.spec.barrier); // shells are for BARRIERS only
  if (rig) {
    ...
    for (const m of rig.mats) { m.emissive.copy(rigGlowColor); m.emissiveIntensity = gi; }
    ...
    casterShell.visible = casterGlowV > 0.02 && !!(ctx && ctx.spec.barrier);
  }
  ...
  const hemiTarget = HEMI_BASE * (state === 'windup' && ctx && ctx.palName === 'storm' ? 1 - 0.45 * clamp01(stateT / ctx.windup) : 1);
  ...
  if (cineOn && ctx && rig) {
    ...
    const focusTarget = state === 'travel' && !ctx.arch.melee && ctx.spec.archetype === 'bolt'
      ? projPos
      : (state === 'aftermath' && !ctx.self) ? dummy.hitPos : casterChest(tmpB);
    cineFocus.lerp(focusTarget, 1 - Math.exp(-realDt * 3));
    const targetR = (cineShotIn ? 4.6 - comboT * 0.45 : 8.6 - comboT * 1.05) + Math.sin(perfNow * 0.4) * 1.1;
    cineRadius += (targetR - cineRadius) * (1 - Math.exp(-realDt * 7)); // whip between shots
    camera.position.set(cineFocus.x + Math.cos(cineAngle) * cineRadius, (cineShotIn ? 2.1 : 3) - comboT * 0.28 + Math.sin(perfNow * 0.27) * 0.7, cineFocus.z + Math.sin(cineAngle) * cineRadius);
...
for (const s of new Set(Object.values(PAL_STYLE).map((p) => p.sheet))) renderer.initTexture(flipbookSheet(s));

// probe hook for E2E scripts
window.__ab = {
  get state() { return state; },
  get t() { return stateT; },
  get gl() { return glName; },
  get textures() { return renderer.info.memory.textures; },
  get spirits() { return spirits.length; },
  get spec() { return ctx ? ctx.spec : null; },
  get sfxSamples() { return sfx.samples.size; }, // generated-pack health for probes
  get sfxMode() { return sfx.sampleMode; },
  get buffs() { return buffOrbits.size; }, // live persistent-buff count (stacking proofs)
  testSpirit(model) { return spawnSpirit({ model, at: new THREE.Vector3(0, 0, 2), path: 'rise', scale: 1.3, dur: 6 }); },
  composeShot() { // frame the current moment like a cinematographer, per archetype
    if (!ctx || !rig) return;
    ...
    if (state === 'windup' || state === 'release') {
      ...
      pos = [cx + 4.6, 2.0, 5.2]; look = [cx + 0.3, 1.5, 0];
      ...
      pos = [mx - 3.2, 2.3, 7.4]; look = [mx, 1.35, 0];
    } else if (ctx.arch.melee || arch === 'dash') {
      pos = [hp.x - 4.4, 2.5, 5]; look = [hp.x - 0.7, 1.55, 0];
    } else if (arch === 'nova' || arch === 'shout') {
      const nx = ctx.novaCenter ? ctx.novaCenter.x : cx;
      pos = [nx + 3, 4.8, 8.2]; look = [nx, 1.1, 0];
    } else if (ctx.self || arch === 'heal' || arch === 'buff' || arch === 'summon') {
      pos = [cx + 4.6, 2.2, 5.4]; look = [cx + 0.3, 1.8, 0];
    } else if (state === 'channel') {
      // wide enough that BOTH actors and the full beam live in the frame
      const mx = (cx + hp.x) / 2;
      pos = [mx, 3.4, 14.5]; look = [mx, 1.5, 0];
    } else {
      pos = [hp.x - 5.6, 2.7, 6.4]; look = [hp.x - 1.2, 1.7, 0];
    }
    camera.position.set(pos[0], pos[1], pos[2]);
    camera.lookAt(look[0], look[1], look[2]);
  },
  prewarmSpirits() { return Promise.all(Object.values(SPIRIT_URLS).map((u) => loadGlb(u).catch(() => null))); },
  get current() { return ctx ? { cls: ctx.cls, id: ctx.id, archetype: ctx.spec.archetype } : null; },
  list() { return Object.fromEntries(Object.entries(CATALOG).map(([c, l]) => [c, l.map((a) => a.id)])); },
  setAbility(cls, id) { return setAbility(cls, id); },
  shot() { composer.render(); return renderer.domElement.toDataURL('image/png'); },
};
