...
for (const overlay of ['diversity_patches.json', 'unveil_patches.json', 'accent_patches.json', 'signature_patches.json', 'round2_patches.json', 'round3_new_specs.json', 'round3_spirit_policy.json', 'round4_semantics.json', 'round5_buff_slots.json', 'round5_buff_dna.json', 'round6_talent_sigs.json']) {
  ...
  try {
    ...
    for (const [id, rawPatch] of Object.entries(patches)) {
      ...
      for (const [k, v] of Object.entries(patch)) {
        if (v && typeof v === 'object' && !Array.isArray(v)) merged[id][k] = { ...(merged[id][k] ?? {}), ...v };
        else merged[id][k] = v;
      }
  ...
  } catch { console.log(`no ${overlay} — skipped`); }
}
...
const PINS = {
  ghost_wolf: { windupStyle: 'none', spirit: { model: 'wolf', path: 'circle', at: 'caster', scale: 0.9, dur: 2.2 } },
...
const ARCH = new Set(['bolt', 'burst', 'strike', 'nova', 'beam', 'dot', 'heal', 'buff', 'shout', 'summon', 'cc', 'dash']);
const PAL = new Set(['physical', 'fire', 'frost', 'arcane', 'shadow', 'holy', 'nature', 'storm', 'blood', 'moon', 'venom', 'gold']);
...
const LOCKED = {
  lightning_bolt: {
    archetype: 'bolt', palette: 'storm', windup: 1.7, power: 1.25, windupStyle: 'runes',
...
const allIds = new Set(Object.values(catalog).flat().map((a) => a.id));
const problems = [];
const out = {};
for (const [id, raw] of Object.entries(merged)) {
  if (!allIds.has(id)) { problems.push(`unknown ability id: ${id}`); continue; }
  const { _cls, notes, ...spec } = raw;
  if (!ARCH.has(spec.archetype)) { problems.push(`${id}: bad archetype ${spec.archetype}`); continue; }
  if (spec.palette && !PAL.has(spec.palette)) { problems.push(`${id}: bad palette ${spec.palette}`); delete spec.palette; }
...
for (const [cls, list] of Object.entries(catalog)) {
  ...
  for (const a of list) {
    ...
    body += `  ${a.id}: ${JSON.stringify(clean)},${_notes ? ` // ${String(_notes).replace(/\n/g, ' ').slice(0, 110)}` : ''}\n`;
  }
}
body += '};\n';
