...
page.on('console', (m) => {
  if (m.type() === 'error') { errors.push({ at: currentLabel, text: m.text() }); console.log(`[console.error @ ${currentLabel}]`, m.text().slice(0, 200)); }
});
page.on('pageerror', (e) => { errors.push({ at: currentLabel, text: e.message }); console.log(`[pageerror @ ${currentLabel}]`, e.message.slice(0, 300)); });

console.log('loading', URL);
await page.goto(URL, { waitUntil: 'domcontentloaded', timeout: 60000 });
await page.waitForFunction('window.__ab !== undefined && window.__ab.current !== null', { timeout: 90000, polling: 200 });
console.log('gallery up; gl =', await page.evaluate(() => window.__ab.gl));

async function waitPhase(id, st, minT, timeout = 210000) { // SwiftShader runs the sim at ~15% speed — long windups need headroom
  await page.waitForFunction(
    (id2, s, mt) => window.__ab && window.__ab.current && window.__ab.current.id === id2 && window.__ab.state === s && window.__ab.t >= mt,
    { timeout, polling: 100 },
    id, st, minT
  );
}
...
async function runAbility(cls, id, shots) {
  ...
  try {
    ...
    else if (arch === 'shout') await waitPhase(id, 'aftermath', 0.22); // ring wavefront mid-expansion
    else if (arch === 'bolt' && (spec?.bolt?.volley || spec?.bolt?.tracer)) await waitPhase(id, 'aftermath', 0.15); // barrage/tracer still in the air
    else if (arch === 'bolt' || arch === 'burst' || arch === 'dot' || arch === 'nova') await waitPhase(id, 'aftermath', 0.28); // flipbook burst at full bloom
    else if (arch === 'summon') await waitPhase(id, 'aftermath', 0.8); // the summoned creature must be RISEN
    ...
    errors.push({ at: currentLabel, text: 'TIMEOUT waiting for phases: ' + e.message.slice(0, 120) });
