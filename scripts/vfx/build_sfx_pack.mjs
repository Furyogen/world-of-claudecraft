...
for (const f of readdirSync(SFXDIR).sort()) {
  ...
  if (buf.length < 2000) {
    console.log(`skip ${f}, suspiciously small (${buf.length}B)`);
    continue;
  }
  pack[m[1]] ??= [];
...
if (!files) {
  console.error('no takes found in', SFXDIR, '- run _tmp_gen_sfx.mjs first');
  process.exit(1);
}
const json = JSON.stringify(pack);
writeFileSync('sfx_pack.json', json);
console.log(
  `sfx_pack.json: ${Object.keys(pack).length} sounds, ${files} takes, ${(bytes / 1048576).toFixed(2)}MB raw -> ${(json.length / 1048576).toFixed(2)}MB json`,
);
