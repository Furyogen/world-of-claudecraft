...
for (const [cls, id] of PERSIST) {
  ...
  try {
    ...
    await page.waitForFunction(
      ...
      { timeout: 240000, polling: 150 },
      id,
    );
    await snap(`buff_persist_${id}`);
    console.log('ok persist', cls, id);
  } catch {
    console.log('FAIL persist', cls, id, '(orbit died early or never registered)');
    bad++;
  }
}
...
await page.waitForFunction(
  'window.__ab.current && window.__ab.current.id === "aspect_of_the_hawk"',
...
try {
  ...
  console.log('FAIL stack, second buff did not coexist with the first');
  bad++;
}
