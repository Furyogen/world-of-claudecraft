...
| Mirror World bespoke creatures & NPCs (`models/creatures/`: `gargoyle.glb`, `poverty_ghost.glb`, `mistshade_lurker.glb`, `hooded_reaper.glb`, `gloomhulk.glb`, `mirrorbound_sentry.glb`, `spirit_unicorn.glb`, `voidfang_stalker.glb`, `gloaming_maw.glb`, and the town NPCs `nerissa.glb`, `morwen.glb`) | World of ClaudeCraft | Project-commissioned: meshed/textured from project-authored concepts via Tripo Studio (Pro), auto-rigged + animated via Tripo Studio's animation library, assembled with `scripts/combine_fbx_to_glb.mjs`. The three quadrupeds (unicorn, voidfang, maw) are walk-cycle only - Tripo's quadruped auto-rig ships a single Walk clip; the two NPCs use a single signature idle. Original assets, not derived from any other game. | Project asset (Tripo Pro output, private) |
