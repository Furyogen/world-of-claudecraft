...
| v02 player bodies and animation tracks (`*_v02.glb` and `*_male_v02.glb`; heads derived from the KayKit Adventurers ranger head) | Commissioned for this project (Levy Street) | in-house | Project asset, rights reserved | **No, permission required** |
...
| Class portrait icons (`public/ui/class-icons/<class>.webp`, all 9 classes, re-encoded from the CraftPix character-icon PNGs to WebP; shown on the pre-game character choose/create screens instead of the 3D model snapshot) | CraftPix | https://craftpix.net | CraftPix premium (royalty-free commercial), purchased by Levy Street account (callum@levystreet.com) | **No, permission required** |
