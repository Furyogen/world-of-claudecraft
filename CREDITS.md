...
| Weirdo Cream Truck rideable mount model (`public/models/mounts/weirdo_cream_truck.glb`) | World of ClaudeCraft | Original project art, procedurally authored under `scripts/assets/weirdo_cream_truck/`. Its rear-shutter portrait composites an owner-supplied photograph of the repository owner's own player character, contributed for this use; the source image and its provenance record are committed under `docs/design/weirdo-cream-truck/reference/` and are inputs to the asset's source fingerprint | Project asset, rights reserved | **No, permission required** |
...
| Weirdo Cream Truck engine and chime (`mount_engine_weirdo_cream_truck.mp3`, `mount_chime_weirdo_cream_truck.mp3`) | World of ClaudeCraft | Original deterministic synthesis via `scripts/gen_weirdo_cream_truck_sfx.mjs` over the sample-level synth in `scripts/sfx/weirdo_cream_audio.mjs`; no sampled source material | Project asset | With the project only |
