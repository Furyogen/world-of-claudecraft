...
| `admin_commands.ts`/`admin_tools_service.ts` | the game-master toolkit beside the moderation pair, gated on `gm.tools` (never the moderator bundle): the pure parser for `/invisible` `/visible` `/freeze` `/unfreeze` `/tpto` `/tptome` `/tp` / the policy service behind an `AdminToolsHost`, wired into `GameServer`. World effects (cloak, ice-block hold, teleports, the persisted `state.adminFreeze`) live on the host in `game.ts`; the sim state is `src/sim/admin_cloak.ts` + `src/sim/admin_freeze.ts` |
