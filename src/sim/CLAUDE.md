...
power coefficients), `melee_ap.ts` (its melee twin: which primary attributes convert to
melee attack power per class, plus the druid form bonuses; the ONE source `recalcPlayerStats`
and the character sheet both read, so the displayed breakdown cannot drift from the sim),
`stun_dr.ts` (CC diminishing-return categories), `item_level.ts`/
