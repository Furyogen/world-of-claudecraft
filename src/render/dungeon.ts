...
import {
  authoredLiftAt,
  doorRampHalf,
  authoredWallSegments as riftAuthoredWallSegments,
...
export class DungeonInteriors {
  ...
  async buildInterior(
    ...
    // Our Molten Abyss (infernal_abyss) also uses an authored room graph, but it
    // renders through the single-shell path below plus its bespoke infernal
    // dressing, so it is deliberately excluded from this procedural-rift branch.
    if (layout.rooms && variant !== 'infernal_abyss') {
      if (!opts?.captureOnly) await ensureInfernalDecorAssets();
      this.placeAuthoredFloor(p, layout, variant);
      this.placeRiftAuthoredWalls(p, layout, variant);
  ...
  private placeRiftAuthoredWalls(p: Placements, layout: DungeonLayout, variant: Variant): void {
    const rooms = layout.rooms ?? [];
    const bannerEvery = variant === 'crypt' ? 4 : 3;
    const openAt = (x: number, z: number): boolean =>
      rooms.some((r) => x > r.x0 && x < r.x1 && z > r.z0 && z < r.z1);
    let i = 0;
    for (const seg of riftAuthoredWallSegments(rooms, layout.doors ?? [])) {
