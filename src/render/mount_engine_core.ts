...
//
// Pure and Node-tested (tests/mount_engine_core.test.ts): no Three, no DOM, no
...
export interface MountEngineState {
  /** Fractional chugs owed since the last firing. */
  accum: number;
}

export function createMountEngineState(): MountEngineState {
  return { accum: 0 };
}

export interface MountEngineInput {
...
export interface MountEngineTick {
 ...
 */
export function stepMountEngine(state: MountEngineState, input: MountEngineInput): MountEngineTick {
