...
export interface InstanceMusicInput {
   ...
   *  map-wide pick. Overrides the open-world zone theme only; dungeons, the
   *  raid arena, and the Sowfield keep their own themes. */
  authoredZone?: MusicZone | null;
}
...
export function instanceMusicDecision(input: InstanceMusicInput): InstanceMusicDecision {
  ...
  const zone = atSowfield
    ? 'vale_cup'
    : ((input.inDungeon || inRaidArena ? null : (input.authoredZone ?? null)) ??
