...
class Sfx {
  ...
  // Authored positional point sounds, keyed by node index (playtest lists are
  // static). Own map so their per-node panners never clash with ambience/casts.
  private pointSlots = new Map<number, LoopSlot>();
  private ready = false;
  ...
  /**
   * Close the AudioContext and drop decoded buffers + active voices on page
   * teardown, so the editor↔playtest navigation ping-pong does not leak an audio
   * thread and its sample buffers per hop. Nulls the context so a later init()
   * rebuilds and re-preloads it (the `if (this.ctx)` guard clears).
   */
  close(): void {
    const ctx = this.ctx;
    this.ctx = null;
