...
function composeTownAtlantis(): Theme {
  const ev: NoteEvent[] = [];
  // A minor nocturne under glass: slow harp lattices, low choir glow, and tiny
  // bell glints like drifting lume-motes — a town asleep beneath the Undertide.
...
function composeAbyss(): Theme {
  const ev: NoteEvent[] = [];
  // E aeolian deep-water drift: long choir swells over slow bass pulses, a
  // sparse harp, and a low reed line — the dark shelf outside the lamplight.
  const Em = { root: 52, minor: true },
    C = { root: 60 },
    Am = { root: 57, minor: true };
  const G = { root: 55 },
...
//   town_atlantis 7 → A (Atlantis nocturne is A minor)
//   abyss 2 → E (the shelf drift is E aeolian)
...
function buildMusicThemes(): Record<string, Theme> {
  return {
    ...
    town_atlantis: composeTownAtlantis(),
    vale: composeVale(),
