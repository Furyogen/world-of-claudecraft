...
every new piece of conquerable content) follows; `design/item-level-ladder.md` is the
living reference for how gear gets its item level and stat budget, including the
recipe for extending the ladder to the next level cap (its companion
`design/ilvl-squish-feasibility.md` is the historical measurement, not source of
truth). `design/design-language/` holds the
