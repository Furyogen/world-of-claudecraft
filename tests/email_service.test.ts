...
describe('EmailService.send', () => {
  it('renders and delivers a transactional email exactly once and logs ok', async () => {
    ...
    expect(log).toHaveBeenCalledWith(
      expect.objectContaining({ event: 'password_changed', ok: true, accountId: 7 }),
    );
  });
  ...
  it('drops a marketing send for an opted-out account and logs the opt-out', async () => {
    ...
    const result = await svc.send({
      ...
      data: { username: 'A', heading: 'News', body: 'Patch notes' },
    });
    expect(result).toBe('skipped');
    expect(sent).toHaveLength(0);
    expect(log).toHaveBeenCalledWith(
      expect.objectContaining({ category: 'marketing', ok: false, error: 'opt-out' }),
    );
  });
