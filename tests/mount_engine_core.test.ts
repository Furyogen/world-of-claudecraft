// The vehicle-mount engine cadence: src/render/mount_engine_core.ts.
//
...
import {
  ...
  type MountEngineInput,
  mountThrottle,
  mountTopSpeed,
  stepMountEngine,
} from '../src/render/mount_engine_core';
...
function chugsOver(seconds: number, input: Omit<MountEngineInput, 'dt'>, dt = 1 / 60): number {
  const state = createMountEngineState();
  let total = 0;
  for (let elapsed = 0; elapsed < seconds - 1e-9; elapsed += dt) {
    total += stepMountEngine(state, { ...input, dt }).chugs;
  }
  return total;
}
...
describe('engine cadence', () => {
  ...
  it('raises pitch and level with speed', () => {
    const state = createMountEngineState();
    const idle = stepMountEngine(state, {
      speed: 0,
      topSpeed: TRUCK_TOP,
      airborne: false,
      dt: 1 / 60,
    });
    const flat = stepMountEngine(createMountEngineState(), {
      speed: TRUCK_TOP,
      topSpeed: TRUCK_TOP,
      airborne: false,
      dt: 1 / 60,
  ...
  it('revs up and thins out off the ground', () => {
    ...
    const grounded = stepMountEngine(createMountEngineState(), { ...input, airborne: false });
    const flying = stepMountEngine(createMountEngineState(), { ...input, airborne: true });
  ...
  it('never machine-guns after a long frame', () => {
    ...
    const tick = stepMountEngine(state, {
      speed: TRUCK_TOP,
      topSpeed: TRUCK_TOP,
      airborne: false,
      dt: 2,
  ...
  it('banks no debt from a negative or zero dt', () => {
    const state = createMountEngineState();
    expect(
      stepMountEngine(state, { speed: 5, topSpeed: TRUCK_TOP, airborne: false, dt: -3 }).chugs,
    ).toBe(0);
  ...
  it('responds to a speed change on the very next step, with no smoothing lag', () => {
    const state = createMountEngineState();
    const slow = stepMountEngine(state, {
      speed: 0,
      topSpeed: TRUCK_TOP,
      airborne: false,
      dt: 1 / 60,
    });
    const fast = stepMountEngine(state, {
      speed: TRUCK_TOP,
      topSpeed: TRUCK_TOP,
      airborne: false,
      dt: 1 / 60,
