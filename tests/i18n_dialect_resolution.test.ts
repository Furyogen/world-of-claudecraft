...
import { en, en_CA, es, es_ES, fr_CA, fr_FR } from '../src/ui/i18n.resolved.generated';
    ...
    describe(`${d.name} over ${d.base}`, () => {
      it('applies every overlay key on top of the base (override wins, diverges from base)', () => {
        ...
        for (const k of keys) {
          ...
          expect(
            get(d.resolved, k),
            `${d.name} overlay key ${k} must win in the resolved table`,
          ).toBe(overlayValue);
          expect(
            get(d.resolved, k),
      ...
      it('falls through every omitted key to the base value, never to en directly', () => {
        ...
        for (const k of omitted) {
          ...
          expect(
            resolved,
            `${d.name} omitted key ${k} must resolve to its ${d.base} base value`,
          ).toBe(baseVal);
          if (baseVal !== get(en, k)) {
            baseDivergesFromEn++;
            expect(
