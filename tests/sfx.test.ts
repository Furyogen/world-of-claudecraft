...
// stride (mount_run_<key>). A VEHICLE mount has no gait, so it plays a
// cylinder-firing chug at an interval the renderer derives from ground speed
// (mount_engine_<key>) and may also carry a signature chime. Every mount is in
// exactly one of these sets, which is what the coverage tests below assert.
const ENGINE_MOUNTS = ['weirdo_cream_truck'] as const;
...
const GAIT_MOUNTS = MOUNT_KEYS.filter(
  (key): key is Exclude<MountKey, (typeof ENGINE_MOUNTS)[number]> =>
    !(ENGINE_MOUNTS as readonly string[]).includes(key),
);
...
describe('mount running audio', () => {
  it('gives every catalog mount exactly one of the two audio shapes', () => {
    for (const mountKey of MOUNT_KEYS) {
      ...
      const hasEngine = `mount_engine_${mountKey}` in SFX_CLIPS;
      expect(hasStride !== hasEngine, `${mountKey} must be gait XOR vehicle`).toBe(true);
      expect(hasEngine).toBe((ENGINE_MOUNTS as readonly string[]).includes(mountKey));
    }
  });
  ...
  it('ships one non-empty MP3 asset per mount cue and no orphan mount clips', () => {
    ...
    const expected = [
      ...GAIT_MOUNTS.map((mountKey) => `mount_run_${mountKey}.mp3`),
      ...ENGINE_MOUNTS.map((mountKey) => `mount_engine_${mountKey}.mp3`),
  ...
  it('plays the engine chug and the chime for a vehicle mount, at the asked rate', () => {
    const buffers = (sfx as unknown as { buffers: Map<string, { duration: number }> }).buffers;
    for (const mountKey of ENGINE_MOUNTS) {
      nowT += 0.5;
      sfx.mountEngine(0, 0, 0, mountKey, 1.31, 0.47, true);
      const engine = sources.at(-1)!;
      expect(engine.buffer).toBe(buffers.get(`mount_engine_${mountKey}`));
  ...
  it('stays silent for a mount that has no engine or chime cue', () => {
    const before = sources.length;
    nowT += 0.5;
    sfx.mountEngine(0, 0, 0, 'valorsteed', 1, 0.5, true);
