...
import { isSupportedLanguage, supportedLanguages } from '../src/ui/i18n';
...
function collectLeaves(
  ...
  if (Array.isArray(enNode)) {
    enNode.forEach((v, i) =>
      collectLeaves(v, (xaNode as unknown[] | undefined)?.[i], `${p}[${i}]`, out),
    );
    return;
  }
...
describe('en_XA generator output (game)', () => {
  ...
  it('accent-pushes the literal text (no plain ASCII letters survive outside placeholders)', () => {
    for (const [path, enLeaf, xaLeaf] of leaves) {
      ...
      expect(ASCII_LETTER.test(xaText), `un-accented ASCII letters at ${path}: ${xaLeaf}`).toBe(
        false,
      );
    }
  });
  ...
  it('transforms the admin catalog with brackets + preserved placeholders', () => {
    ...
    for (const [path, enLeaf, xaLeaf] of leaves) {
      ...
      expect(
        ...
        `admin en_XA leaf not bracketed at ${path}`,
      ).toBe(true);
      expect(s.match(PLACEHOLDER) ?? [], `admin placeholders changed at ${path}`).toEqual(
        enLeaf.match(PLACEHOLDER) ?? [],
      );
    }
  });
});
