...
describe('character typeahead search', () => {
  it('adds the realm column before indexes that depend on it', () => {
    ...
    expect(SCHEMA.indexOf('ADD COLUMN IF NOT EXISTS realm')).toBeLessThan(
      SCHEMA.indexOf('CREATE INDEX IF NOT EXISTS characters_lifetime_xp'),
    );
  });
  ...
  it('renames existing case-colliding characters before adding the folded unique index', () => {
    ...
    expect(SOCIAL_SCHEMA).toContain(
      'row_number() OVER (PARTITION BY realm, lower(name) ORDER BY created_at, id)',
    );
    expect(SOCIAL_SCHEMA).toContain('force_rename = TRUE');
    expect(
      SOCIAL_SCHEMA.indexOf(
        'dedupe case-insensitive character names before adding the unique index',
      ),
    ).toBeLessThan(
      SOCIAL_SCHEMA.indexOf('CREATE UNIQUE INDEX IF NOT EXISTS characters_realm_lower_name_unique'),
    );
  });
  ...
  it('uses the lower-name prefix predicate and preserves wildcard escaping', async () => {
    ...
    await expect(searchCharacters('  Al%_  ', 99)).resolves.toEqual([
      { name: 'Al%_', cls: 'mage', level: 12 },
    ]);
